...
int main() {
  ...
  r = SymCryptMlKemkeyGetValue(k, public_key, sizeof(public_key),
                               SYMCRYPT_MLKEMKEY_FORMAT_ENCAPSULATION_KEY, 0);
  ...
  r = SymCryptMlKemEncapsulateEx(
      k, encapsulation_seed, sizeof(encapsulation_seed), shared_secret,
      sizeof(shared_secret), ciphertext, sizeof(ciphertext));
  ...
  SymCryptMlKemDecapsulate(k, ciphertext, sizeof(ciphertext), shared_secret2, sizeof(shared_secret2));
