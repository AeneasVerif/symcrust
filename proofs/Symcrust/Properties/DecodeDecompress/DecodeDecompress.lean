...
theorem decode_coefficient.early_load_progress_spec (b : Aeneas.Std.Slice U8) (d : U32) (f : Std.Array U16 256#usize)
  ...
  . simp only [Array.getElem!_Nat_eq, Slice.getElem!_Nat_eq] at a_post
    ...
    have h3 :
      ∀ hCast : 8 * (List.map U8.bv ↑a).length = 32, accumulator1.val =
      (BitVec.cast hCast (BitVec.fromLEBytes (List.map U8.bv ↑a)) : BitVec 32).toNat := by
      simp [UScalar.val, accumulator1_post]
    have h4 :
    ...
    . simp_scalar
      have :
        ...
        intro j
        dcases hj : j < d
        . have := SpecAux.testBit_of_sum d (by omega) j hj
          simp_lists_scalar [this, BitVec.fromLEBytes_getElem!, Slice.getElem!_Nat_eq]
          rw [a_post (j / 8) (by scalar_tac)]
          have := hinv.2.1 -- Used by `scalar_tac` in the subgoals generated by `congr 1`
          congr 1
          . congr
            scalar_tac
          . scalar_tac
        . simp_scalar
          have : (∑ x : Fin d.val,
            b[(d.val * i' + x.val) / 8]!.bv[(d.val * i' + x.val) % 8]!.toNat * 2 ^ x.val).testBit j = false := by
            apply Nat.testBit_eq_false_of_lt
...
theorem decode_coefficient.late_load_progress_spec (b : Aeneas.Std.Slice U8) (d : U32) (f : Std.Array U16 256#usize)
  ...
  have hnum_bytes_read : ↑num_bytes_read + 3 < b.length := by
    ...
    rw [(by omega : num_bytes_read.val = 4 * k), hb2]
    have : 4 * k < 32 * d.val := by
      -- **TODO** Even though `simp_scalar` can solve both the following `have` statement and the overall goal
      -- given the `have` statement, it can't solve the overall goal all in one go
  ...
  . let* ⟨bits_to_decode1, hbits_to_decode1⟩ ← UScalar.and_spec
    ...
    . ext j hj
      ...
      . simp_lists
        ...
        . congr 1
          . have : 1 <<< (d.val - n_bits_in_accumulator.val) % U32.size = 1 <<< (d.val - n_bits_in_accumulator.val) := by
              simp_scalar
            ...
            . rw [← BitVec.getElem!_eq_testBit_toNat, ← BitVec.getElem!_eq_testBit_toNat,
              ...
              . congr
                rw [haccumulator1 (k / 8) (by scalar_tac), List.flatMap_eq_map, List.getElem!_slice,
                  List.getElem!_map_eq] <;> simp_lists_scalar
              . scalar_tac
        ...
        . rw [SpecAux.testBit_of_sum d.val (by omega) j hj1,
          ...
          . dcases hj2 : j < n_bits_in_accumulator.val
            . rw [SpecAux.testBit_of_add1 _ _ d.val n_bits_in_accumulator.val j (by omega) (by omega) hj2]
              all_goals
                have := hinv.2.1 -- Need this for `simp_scalar [*]` to succeed
                simp_scalar [*]
            . rw [SpecAux.testBit_of_add2 _ _ d.val n_bits_in_accumulator.val j (by omega) (by omega) hj1]
              . have : (1 <<< (↑d - ↑n_bits_in_accumulator) - 1).testBit (j - ↑n_bits_in_accumulator) := by
                ...
                simp only [BitVec.toNat_cast, Slice.getElem!_Nat_eq]
...
/-- This function is meant to reason about the output of `decode_coefficient` when it is not necessary to load
    ...
    specifies that the initial number of bits in the accumulator is not 0 (meaning it is not necessary to load
...
theorem decode_coefficient.no_load_progress_spec (b : Aeneas.Std.Slice U8) (d : U32) (f : Std.Array U16 256#usize)
  ...
  (hn_bits_to_decode : n_bits_to_decode.val = Min.min d.val n_bits_in_accumulator.val) (i' : U32)
  (hi' : ↑i' = 1 <<< ↑n_bits_to_decode % U32.size) (i1 : U32)
  (hi1 : ↑i1 = i'.val - 1) (bits_to_decode : U32) (hbits_to_decode : ↑bits_to_decode = (acc &&& i1).val)
  (accumulator1 : U32) (haccumulator1 : ↑accumulator1 = acc.val >>> ↑n_bits_to_decode)
  (n_bits_in_accumulator1 : U32)
  (hn_bits_in_accumulator1 : ↑n_bits_in_accumulator1 = n_bits_in_accumulator.val - ↑n_bits_to_decode)
  (coefficient1 : U32) (hd : ¬d > n_bits_to_decode) (hi'_1 : 1 ≤ i'.val)
  (haccumulator1_1 : accumulator1.bv = acc.bv >>> n_bits_to_decode.val)
  (hcoefficient1 : ↑coefficient1 = @UScalar.val UScalarTy.U32
    (@HOr.hOr U32 U32 U32 instHOrUScalar (UScalar.ofNat 0 (by decide)) bits_to_decode))
  :
  ∃ num_bytes_read' acc' n_bits_in_accumulator' coefficient,
    ok (num_bytes_read, accumulator1, n_bits_in_accumulator1, coefficient1) =
      ...
      @SpecAux.Stream.decode.body d.val 4
          ...
          { F := (poly_to_vector (to_poly f)).set (↑i) (↑coefficient) hi, num_bytes_read := ↑num_bytes_read',
            acc := ↑↑acc', num_bits_in_acc := ↑n_bits_in_accumulator' } ∧
        Stream.decode.length_inv (↑d) 4 (↑num_bytes_read') (↑n_bits_in_accumulator') (↑i + 1) ∧
          (∀ j < ↑n_bits_in_accumulator',
              acc'.val.testBit j =
                b[(8 * ↑num_bytes_read' - ↑n_bits_in_accumulator' + j) / 8]!.val.testBit
                  ((8 * ↑num_bytes_read' - ↑n_bits_in_accumulator' + j) % 8)) ∧
  ...
  . have hmodu32 : 1 <<< n_bits_to_decode.val % U32.size = 1 <<< n_bits_to_decode.val := by simp_scalar
    ...
    have : acc.val &&& 1 <<< n_bits_to_decode.val - 1 =
      ...
      intro j
      dcases hj : j < d
      . have := SpecAux.testBit_of_sum d.val (by omega) j hj
        simp_scalar [*]
      . rw [Nat.testBit_eq_false_of_lt, Nat.testBit_eq_false_of_lt]
        . apply Nat.lt_of_lt_of_le _ (by scalar_tac +nonLin : 2 ^ d.val ≤ 2 ^ j)
...
theorem decode_coefficient.no_early_load_progress_spec (b : Aeneas.Std.Slice U8) (d : U32)
  ...
  (hacc1 : (∀ j < n_bits_in_accumulator.val, acc.val.testBit j =
    b[(8 * num_bytes_read.val - n_bits_in_accumulator.val + j) / 8]!.val.testBit
      ((8 * num_bytes_read.val - n_bits_in_accumulator.val + j) % 8)))
  (hacc2 : ∀ j ∈ [↑n_bits_in_accumulator: 32], acc.val.testBit j = false)
  (hinv : Stream.decode.length_inv (↑d) 4 ↑num_bytes_read ↑n_bits_in_accumulator ↑i)
  (hb1 : ∀ i < 256, ∑ a : Fin d.val,
  ...
  . simp only [mem_std_range_step_one]
    apply decode_coefficient.late_load_progress_spec b d f num_bytes_read acc n_bits_in_accumulator i
      ...
      hi1 bits_to_decode hbits_to_decode n_bits_in_accumulator1 hn_bits_in_accumulator1
      coefficient1 <;> assumption
  . apply decode_coefficient.no_load_progress_spec b d f num_bytes_read acc n_bits_in_accumulator i
      hacc1 hacc2 hinv hb1 hb2 hi hd hn_bits_in_accumulator n_bits_to_decode hn_bits_to_decode i' hi' i1
      hi1 bits_to_decode hbits_to_decode accumulator1 haccumulator1 n_bits_in_accumulator1
...
def decode_coefficient.progress_spec (b : Aeneas.Std.Slice U8) (d : U32) (f : Array U16 256#usize)
  ...
  . exact decode_coefficient.no_early_load_progress_spec b d f num_bytes_read acc n_bits_in_accumulator i
      hacc1 hacc2 hinv hb1 hb2 hi hd (by assumption)
    ...
    . let* ⟨i2, hi2⟩ ← U32.sub_bv_spec
      ...
      . let* ⟨coefficient4, h4⟩ ← U32.ShiftRight_IScalar_spec
        ...
        . apply Exists.intro i3
          simp [hpe_dst1, hi3, h4, h3, h2, hi1, hi2]
  . next hd =>
...
def poly_element_decode_and_decompress_loop.progress_spec (b : Aeneas.Std.Slice U8) (d : U32)
  ...
  . let* ⟨num_bytes_read', acc', n_bits_in_accumulator', coefficient, h1, h2, h3⟩ ←
    ...
    simp_all
    fcongr
    simp only [poly_to_vector, Spec.Q, to_poly, Fin.getElem!_fin, Array.getElem!_Nat_eq]
    ext
    next j hj =>
    dcases i = j <;> simp_lists
...
def poly_element_decode_and_decompress.spec (b : Aeneas.Std.Slice U8) (d : U32) (f : Array U16 256#usize)
  ...
  . apply Exists.intro res.1
    ...
    have heq : poly_to_vector (to_poly f) = Vector.replicate 256 0 := by
      ...
      next i hi =>
      simp_lists
      simp only [Array.getElem!_Nat_eq] at hf
      simp [hf i hi]
    rw [heq]
    rfl
