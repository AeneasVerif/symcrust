...
import Symcrust.Properties.DecodeDecompress.TargetDecode
...
`Nist spec ⟷₁ Lean spec (monadic) ⟷₂ Lean spec (functional) ⟷₃ Auxiliary spec ⟷₄ Aeneas translation`
  - In the above verification pipeline:
    ...
    - `Auxiliary spec` corresponds to `Stream.decode`.
    - `⟷₂` corresponds to `Target.byteDecode.eq_spec`.
    - `⟷₃` is bundled together with `⟷₂` in the form of `Stream.decode.spec`.
