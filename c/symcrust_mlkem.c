/* 
  This file was generated by KaRaMeL <https://github.com/FStarLang/karamel>
  KaRaMeL invocation: /Users/jonathan/Code/eurydice/eurydice symcrust.llbc --output c/ -fcomments --config c.yaml
  F* version: <unknown>
  KaRaMeL version: 734ff7e1
 */

#include "internal/symcrust_mlkem.h"

#include "symcrust_hash.h"
#include "symcrust_common.h"
#include "internal/symcrust_hash.h"
#include "internal/symcrust_common.h"

/**
This function found in impl {core::ops::try_trait::FromResidual<core::result::Result<core::convert::Infallible, symcrust::common::Error>> for symcrust::common::Error}
*/
symcrust_common_Error symcrust_error_from_residual_54(core_result_Result_b5 r)
{
  if (!(r.tag == core_result_Ok))
  {
    symcrust_common_Error e = r.val.case_Err;
    return e;
  }
  return symcrust_common_Error_NoError;
}

#define MATRIX_MAX_NROWS ((size_t)4U)

static uint32_t min(uint32_t x, uint32_t y)
{
  uint32_t uu____0;
  if (x <= y)
  {
    uu____0 = x;
  }
  else
  {
    uu____0 = y;
  }
  return uu____0;
}

#define Q (3329U)

static KRML_MUSTINLINE uint32_t mod_reduce(uint32_t a)
{
  EURYDICE_ASSERT(a < 2U * Q, "panic!");
  uint32_t res = core_num__u32__wrapping_sub(a, Q);
  if (!(res >> 16U == 0U))
  {
    EURYDICE_ASSERT(res >> 16U == 65535U, "panic!");
  }
  uint32_t res0 = core_num__u32__wrapping_add(res, Q & res >> 16U);
  EURYDICE_ASSERT(res0 < Q, "panic!");
  return res0;
}

static symcrust_common_Error
poly_element_decode_and_decompress(
  Eurydice_slice pb_src,
  uint32_t n_bits_per_coefficient,
  Eurydice_arr_bc *pe_dst
)
{
  /* _In_reads_bytes_(n_bits_per_coefficient*(MLWE_POLYNOMIAL_COEFFICIENTS / 8)) */
  size_t cb_src_read = (size_t)0U;
  uint32_t accumulator = 0U;
  uint32_t n_bits_in_accumulator = 0U;
  EURYDICE_ASSERT(n_bits_per_coefficient > 0U, "panic!");
  EURYDICE_ASSERT(n_bits_per_coefficient <= 12U, "panic!");
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t coefficient = 0U;
    uint32_t n_bits_in_coefficient = 0U;
    while (true)
    {
      if (n_bits_in_accumulator == 0U)
      {
        /* first gather and decode bits from pb_src
          FIXME
          *accumulator = u32::from_le_bytes(&pb_src[*cb_src_read..*cb_src_read+4]).try_into().unwrap());
          FIXME
          *accumulator = u32::from_le_bytes(&pb_src[*cb_src_read..*cb_src_read+4]).try_into().unwrap()); */
        Eurydice_arr_e9
        uu____0 = symcrust_ntt_slice_to_sub_array((size_t)4U, pb_src, cb_src_read, Eurydice_arr_e9);
        accumulator = core_num__u32__from_le_bytes(uu____0);
        cb_src_read = cb_src_read + (size_t)4U;
        n_bits_in_accumulator = 32U;
        uint32_t
        n_bits_to_decode =
          min(n_bits_per_coefficient - n_bits_in_coefficient,
            n_bits_in_accumulator);
        EURYDICE_ASSERT(n_bits_to_decode <= n_bits_in_accumulator, "panic!");
        uint32_t bits_to_decode = accumulator & ((1U << (uint32_t)n_bits_to_decode) - 1U);
        accumulator = accumulator >> (uint32_t)n_bits_to_decode;
        n_bits_in_accumulator = n_bits_in_accumulator - n_bits_to_decode;
        coefficient = coefficient | bits_to_decode << (uint32_t)n_bits_in_coefficient;
        n_bits_in_coefficient = n_bits_in_coefficient + n_bits_to_decode;
        if (!(n_bits_per_coefficient > n_bits_in_coefficient))
        {
          break;
        }
      }
      else
      {
        uint32_t
        n_bits_to_decode =
          min(n_bits_per_coefficient - n_bits_in_coefficient,
            n_bits_in_accumulator);
        EURYDICE_ASSERT(n_bits_to_decode <= n_bits_in_accumulator, "panic!");
        uint32_t bits_to_decode = accumulator & ((1U << (uint32_t)n_bits_to_decode) - 1U);
        accumulator = accumulator >> (uint32_t)n_bits_to_decode;
        n_bits_in_accumulator = n_bits_in_accumulator - n_bits_to_decode;
        coefficient = coefficient | bits_to_decode << (uint32_t)n_bits_in_coefficient;
        n_bits_in_coefficient = n_bits_in_coefficient + n_bits_to_decode;
        if (!(n_bits_per_coefficient > n_bits_in_coefficient))
        {
          break;
        }
      }
    }
    EURYDICE_ASSERT(n_bits_in_coefficient == n_bits_per_coefficient, "panic!");
    /* decompress the coefficient
      when n_bits_per_coefficient < 12 we decompress per Decompress_d in draft FIPS 203
      otherwise we perform input validation per 203 6.2 Input validation 2 (Modulus check) */
    if (n_bits_per_coefficient < 12U)
    {
      coefficient = coefficient * Q;
      coefficient = coefficient >> (uint32_t)(n_bits_per_coefficient - 1U);
      coefficient++;
      /* final divide by two to get multiplication by Q / 2^n_bits_per_coefficient */
      coefficient = coefficient >> 1U;
      /* modular reduction by conditional subtraction */
      coefficient = mod_reduce(coefficient);
      EURYDICE_ASSERT(coefficient < Q, "panic!");
    }
    else if (coefficient > Q)
    {
      return symcrust_common_Error_InvalidBlob;
    }
    pe_dst->data[i] = (uint16_t)coefficient;
  }
  EURYDICE_ASSERT(n_bits_in_accumulator == 0U, "panic!");
  EURYDICE_ASSERT(cb_src_read ==
      (size_t)(n_bits_per_coefficient * ((uint32_t)SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / 8U)),
    "panic!");
  return symcrust_common_Error_NoError;
}

static symcrust_common_Error
vector_decode_and_decompress(
  Eurydice_slice pb_src,
  uint32_t n_bits_per_coefficient,
  Eurydice_slice pv_dst
)
{
  size_t n_rows = Eurydice_slice_len(pv_dst, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(n_bits_per_coefficient > 0U, "panic!");
  EURYDICE_ASSERT(n_bits_per_coefficient <= 12U, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pb_src, uint8_t) ==
      n_rows * (size_t)n_bits_per_coefficient *
        (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U),
    "panic!");
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    size_t
    pb_src_index =
      i * (size_t)n_bits_per_coefficient * (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
    symcrust_common_Error
    sc_error =
      poly_element_decode_and_decompress(Eurydice_slice_subslice_from(pb_src,
          pb_src_index,
          uint8_t,
          size_t,
          Eurydice_derefed_slice),
        n_bits_per_coefficient,
        &Eurydice_slice_index(pv_dst, i, Eurydice_arr_bc, Eurydice_arr_bc *));
    switch (sc_error)
    {
      case symcrust_common_Error_NoError:
        {
          break;
        }
      default:
        {
          return sc_error;
        }
    }
  }
  return symcrust_common_Error_NoError;
}

/**
A monomorphic instance of Eurydice.arr
with types uint16_t
with const generics
- $128size_t
*/
typedef struct arr_f9_s { uint16_t data[128U]; } arr_f9;

#define ZETA_BIT_REV_TIMES_R ((KRML_CLITERAL(arr_f9){ .data = { 2285U, 2571U, 2970U, 1812U, 1493U, 1422U, 287U, 202U, 3158U, 622U, 1577U, 182U, 962U, 2127U, 1855U, 1468U, 573U, 2004U, 264U, 383U, 2500U, 1458U, 1727U, 3199U, 2648U, 1017U, 732U, 608U, 1787U, 411U, 3124U, 1758U, 1223U, 652U, 2777U, 1015U, 2036U, 1491U, 3047U, 1785U, 516U, 3321U, 3009U, 2663U, 1711U, 2167U, 126U, 1469U, 2476U, 3239U, 3058U, 830U, 107U, 1908U, 3082U, 2378U, 2931U, 961U, 1821U, 2604U, 448U, 2264U, 677U, 2054U, 2226U, 430U, 555U, 843U, 2078U, 871U, 1550U, 105U, 422U, 587U, 177U, 3094U, 3038U, 2869U, 1574U, 1653U, 3083U, 778U, 1159U, 3182U, 2552U, 1483U, 2727U, 1119U, 1739U, 644U, 2457U, 349U, 418U, 329U, 3173U, 3254U, 817U, 1097U, 603U, 610U, 1322U, 2044U, 1864U, 384U, 2114U, 3193U, 1218U, 1994U, 2455U, 220U, 2142U, 1670U, 2144U, 1799U, 2051U, 794U, 1819U, 2475U, 2459U, 478U, 3221U, 3021U, 996U, 991U, 958U, 1869U, 1522U, 1628U } }))

#define ZETA_BIT_REV_TIMES_R_TIMES_NEG_Q_INV_MOD_R ((KRML_CLITERAL(arr_f9){ .data = { 19U, 34037U, 50790U, 64748U, 52011U, 12402U, 37345U, 16694U, 20906U, 37778U, 3799U, 15690U, 54846U, 64177U, 11201U, 34372U, 5827U, 48172U, 26360U, 29057U, 59964U, 1102U, 44097U, 26241U, 28072U, 41223U, 10532U, 56736U, 47109U, 56677U, 38860U, 16162U, 5689U, 6516U, 64039U, 34569U, 23564U, 45357U, 44825U, 40455U, 12796U, 38919U, 49471U, 12441U, 56401U, 649U, 25986U, 37699U, 45652U, 28249U, 15886U, 8898U, 28309U, 56460U, 30198U, 47286U, 52109U, 51519U, 29155U, 12756U, 48704U, 61224U, 24155U, 17914U, 334U, 54354U, 11477U, 52149U, 32226U, 14233U, 45042U, 21655U, 27738U, 52405U, 64591U, 4586U, 14882U, 42443U, 59354U, 60043U, 33525U, 32502U, 54905U, 35218U, 36360U, 18741U, 28761U, 52897U, 18485U, 45436U, 47975U, 47011U, 14430U, 46007U, 5275U, 12618U, 31183U, 45239U, 40101U, 63390U, 7382U, 50180U, 41144U, 32384U, 20926U, 6279U, 54590U, 14902U, 41321U, 11044U, 48546U, 51066U, 55200U, 21497U, 7933U, 20198U, 22501U, 42325U, 54629U, 17442U, 33899U, 23859U, 36892U, 20257U, 41538U, 57779U, 17422U, 42404U } }))

#define RMASK (65535U)

#define NEG_Q_INV_MOD_R (3327U)

#define RLOG2 (16U)

/**
 Computes: ((a * b) / R) % Q

 This simply applies the montgomery reduction to: a * b

 Pre:
  bMont = (b * NegQInvModR) % R
  bMont <= R
*/
static KRML_MUSTINLINE uint32_t mont_mul(uint32_t a, uint32_t b, uint32_t b_mont)
{
  EURYDICE_ASSERT(a < Q, "panic!");
  EURYDICE_ASSERT(b < Q, "panic!");
  EURYDICE_ASSERT(b_mont <= RMASK, "panic!");
  EURYDICE_ASSERT(b_mont == (b * NEG_Q_INV_MOD_R & RMASK), "panic!");
  uint32_t res = a * b;
  uint32_t inv = a * b_mont & RMASK;
  res = res + inv * Q;
  res = res >> (uint32_t)RLOG2;
  return mod_reduce(res);
}

static KRML_MUSTINLINE uint32_t mod_sub(uint32_t a, uint32_t b)
{
  EURYDICE_ASSERT(a < Q, "panic!");
  EURYDICE_ASSERT(b < Q, "panic!");
  uint32_t res = core_num__u32__wrapping_sub(a, b);
  if (!(res >> 16U == 0U))
  {
    EURYDICE_ASSERT(res >> 16U == 65535U, "panic!");
  }
  uint32_t res0 = core_num__u32__wrapping_add(res, Q & res >> 16U);
  EURYDICE_ASSERT(res0 < Q, "panic!");
  return res0;
}

static KRML_MUSTINLINE uint32_t mod_add(uint32_t a, uint32_t b)
{
  EURYDICE_ASSERT(a < Q, "panic!");
  EURYDICE_ASSERT(b < Q, "panic!");
  return mod_reduce(a + b);
}

static void poly_element_ntt_layer_c(Eurydice_arr_bc *pe_src, size_t k, size_t len)
{
  for (size_t start = (size_t)0U; start < (size_t)256U; start = start + (size_t)2U * len)
  {
    uint32_t twiddle_factor = (uint32_t)ZETA_BIT_REV_TIMES_R.data[k];
    uint32_t twiddle_factor_mont = (uint32_t)ZETA_BIT_REV_TIMES_R_TIMES_NEG_Q_INV_MOD_R.data[k];
    k++;
    for (size_t j = (size_t)0U; j < len; j++)
    {
      /* c1TimesTwiddle = ((c1 * (Zeta^BitRev(k) * R)) / R) % Q
                       = ((f(start + j + len) * (Zeta^BitRev(k) * R)) / R) mod Q
                       = (f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1 = (f(start + j) - f(start + j + len) * Zeta^BitRev(k)) mod Q
        c0 = (f(start + j) + f(start + j + len) * Zeta^BitRev(k)) mod Q */
      uint32_t c0 = (uint32_t)pe_src->data[start + j];
      EURYDICE_ASSERT(c0 < Q, "panic!");
      /* c1TimesTwiddle = ((c1 * (Zeta^BitRev(k) * R)) / R) % Q
                       = ((f(start + j + len) * (Zeta^BitRev(k) * R)) / R) mod Q
                       = (f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1 = (f(start + j) - f(start + j + len) * Zeta^BitRev(k)) mod Q
        c0 = (f(start + j) + f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1 = (f(start + j) - f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1TimesTwiddle = ((c1 * (Zeta^BitRev(k) * R)) / R) % Q
                       = ((f(start + j + len) * (Zeta^BitRev(k) * R)) / R) mod Q
                       = (f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1 = (f(start + j) - f(start + j + len) * Zeta^BitRev(k)) mod Q
        c0 = (f(start + j) + f(start + j + len) * Zeta^BitRev(k)) mod Q
        c0 = (f(start + j) + f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1TimesTwiddle = ((c1 * (Zeta^BitRev(k) * R)) / R) % Q
                       = ((f(start + j + len) * (Zeta^BitRev(k) * R)) / R) mod Q
                       = (f(start + j + len) * Zeta^BitRev(k)) mod Q
        c1 = (f(start + j) - f(start + j + len) * Zeta^BitRev(k)) mod Q
        c0 = (f(start + j) + f(start + j + len) * Zeta^BitRev(k)) mod Q */
      uint32_t c1 = (uint32_t)pe_src->data[start + j + len];
      EURYDICE_ASSERT(c1 < Q, "panic!");
      uint32_t c1_times_twiddle = mont_mul(c1, twiddle_factor, twiddle_factor_mont);
      c1 = mod_sub(c0, c1_times_twiddle);
      c0 = mod_add(c0, c1_times_twiddle);
      pe_src->data[start + j] = (uint16_t)c0;
      pe_src->data[start + j + len] = (uint16_t)c1;
    }
  }
}

static KRML_MUSTINLINE void
poly_element_ntt_layer(Eurydice_arr_bc *pe_src, size_t k, size_t len)
{
  poly_element_ntt_layer_c(pe_src, k, len);
}

static void poly_element_ntt(Eurydice_arr_bc *pe_src)
{
  poly_element_ntt_layer(pe_src, (size_t)1U, (size_t)128U);
  poly_element_ntt_layer(pe_src, (size_t)2U, (size_t)64U);
  poly_element_ntt_layer(pe_src, (size_t)4U, (size_t)32U);
  poly_element_ntt_layer(pe_src, (size_t)8U, (size_t)16U);
  poly_element_ntt_layer(pe_src, (size_t)16U, (size_t)8U);
  poly_element_ntt_layer(pe_src, (size_t)32U, (size_t)4U);
  poly_element_ntt_layer(pe_src, (size_t)64U, (size_t)2U);
}

static void vector_ntt(Eurydice_slice pv_src)
{
  size_t n_rows = Eurydice_slice_len(pv_src, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    poly_element_ntt(&Eurydice_slice_index(pv_src, i, Eurydice_arr_bc, Eurydice_arr_bc *));
  }
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
size_t symcrust_key_matrix_len_fd(Eurydice_dst_6f self)
{
  return self.ptr->header.n_rows * self.ptr->header.n_rows;
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice symcrust_key_s_mut_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      m_len + self.ptr->header.n_rows,
      m_len + (size_t)2U * self.ptr->header.n_rows,
      Eurydice_arr_bc *);
}

#define ZETA_TO_TIMES_BIT_REV_PLUS_1_TIMES_R ((KRML_CLITERAL(arr_f9){ .data = { 2226U, 1103U, 430U, 2899U, 555U, 2774U, 843U, 2486U, 2078U, 1251U, 871U, 2458U, 1550U, 1779U, 105U, 3224U, 422U, 2907U, 587U, 2742U, 177U, 3152U, 3094U, 235U, 3038U, 291U, 2869U, 460U, 1574U, 1755U, 1653U, 1676U, 3083U, 246U, 778U, 2551U, 1159U, 2170U, 3182U, 147U, 2552U, 777U, 1483U, 1846U, 2727U, 602U, 1119U, 2210U, 1739U, 1590U, 644U, 2685U, 2457U, 872U, 349U, 2980U, 418U, 2911U, 329U, 3000U, 3173U, 156U, 3254U, 75U, 817U, 2512U, 1097U, 2232U, 603U, 2726U, 610U, 2719U, 1322U, 2007U, 2044U, 1285U, 1864U, 1465U, 384U, 2945U, 2114U, 1215U, 3193U, 136U, 1218U, 2111U, 1994U, 1335U, 2455U, 874U, 220U, 3109U, 2142U, 1187U, 1670U, 1659U, 2144U, 1185U, 1799U, 1530U, 2051U, 1278U, 794U, 2535U, 1819U, 1510U, 2475U, 854U, 2459U, 870U, 478U, 2851U, 3221U, 108U, 3021U, 308U, 996U, 2333U, 991U, 2338U, 958U, 2371U, 1869U, 1460U, 1522U, 1807U, 1628U, 1701U } }))

static void
poly_element_mul_and_accumulate(
  Eurydice_arr_bc *pe_src1,
  Eurydice_arr_bc *pe_src2,
  Eurydice_arr_82 *pa_dst
)
{
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)2U; i++)
  {
    uint32_t a0 = (uint32_t)pe_src1->data[(size_t)2U * i];
    /* debug_assert!( a0 < Q ); */
    uint32_t a1 = (uint32_t)pe_src1->data[(size_t)2U * i + (size_t)1U];
    /* debug_assert!( a1 < Q ); */
    uint32_t b0 = (uint32_t)pe_src2->data[(size_t)2U * i];
    /* debug_assert!( b0 < Q ); */
    uint32_t b1 = (uint32_t)pe_src2->data[(size_t)2U * i + (size_t)1U];
    /* debug_assert!( b1 < Q ); */
    uint32_t c0 = pa_dst->data[(size_t)2U * i];
    /* debug_assert!( c0 <= 3*((3328*3328) + (3494*3312)) ); */
    uint32_t c1 = pa_dst->data[(size_t)2U * i + (size_t)1U];
    /* debug_assert!( c1 <= 3*((3328*3328) + (3494*3312)) );
      multiplication results in range [0, 3328*3328] */
    uint32_t a0b0 = a0 * b0;
    uint32_t a1b1 = a1 * b1;
    uint32_t a0b1 = a0 * b1;
    uint32_t a1b0 = a1 * b0;
    /* we need a1*b1*zetaTwoTimesBitRevPlus1TimesR[i]
      eagerly reduce a1*b1 with montgomery reduction
      a1b1 = red(a1*b1) -> range [0,3494]
        (3494 is maximum result of first step of montgomery reduction of x*y for x,y in [0,3328])
      we do not need to do final reduction yet */
    uint32_t inv = core_num__u32__wrapping_mul(a1b1, NEG_Q_INV_MOD_R) & RMASK;
    /* inv = (a1*b1 * (-Q^(-1) mod R)) % R */
    uint32_t a1b10 = (a1b1 + inv * Q) >> (uint32_t)RLOG2;
    /* a1b1 = (a1*b1 + a1*b1 * (-Q^(-1) mod R) * Q) / R
           = (a1*b1 * R^-1) mod Q
      debug_assert!( a1b1 <= 3494 );
      now multiply a1b1 by power of zeta */
    uint32_t a1b1zetapow = a1b10 * (uint32_t)ZETA_TO_TIMES_BIT_REV_PLUS_1_TIMES_R.data[i];
    /* a1b1zetapow = (a1*b1 * R^-1) * (Zeta^(2*BitRev(i) + 1) * R) mod Q
                  = a1*b1 * Zeta^(2*BitRev(i) + 1) mod Q
      sum pairs of products */
    a0b0 = a0b0 + a1b1zetapow;
    /* debug_assert!( a0b0 <= (3328*3328) + (3494*3312) ); */
    a0b1 = a0b1 + a1b0;
    /* debug_assert!( a0b1 <= 2*3328*3328 );
      We sum at most 4 pairs of products into an accumulator in ML-KEM
      debug_assert!( MATRIX_MAX_NROWS <= 4 ); */
    c0 = c0 + a0b0;
    /* debug_assert!( c0 < (4*3328*3328) + (4*3494*3312) ); */
    c1 = c1 + a0b1;
    /* debug_assert!( c1 < (5*3328*3328) + (3*3494*3312) ); */
    pa_dst->data[(size_t)2U * i] = c0;
    pa_dst->data[(size_t)2U * i + (size_t)1U] = c1;
  }
}

static void
montgomery_reduce_and_add_poly_element_accumulator_to_poly_element(
  Eurydice_arr_82 *pa_src,
  Eurydice_arr_bc *pe_dst
)
{
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t a = pa_src->data[i];
    /* debug_assert!( a <= 4*((3328*3328) + (3494*3312)) ); */
    pa_src->data[i] = 0U;
    /* debug_assert!( c < Q );
      montgomery reduce sum of products
      debug_assert!( a <= 4711 );
      add destination
      debug_assert!( c <= 8039 );
      subtraction and conditional additions for constant time range reduction
      debug_assert!( (c >= ((-2*(Q as i32)) as u32)) || (c < 1381) );
      debug_assert!( (c >= ((-(Q as i32) as u32))) || (c < Q) );
      debug_assert!( c < Q ); */
    uint32_t c = (uint32_t)pe_dst->data[i];
    uint32_t inv = core_num__u32__wrapping_mul(a, NEG_Q_INV_MOD_R) & RMASK;
    a = (a + inv * Q) >> (uint32_t)RLOG2;
    c = c + a;
    c = core_num__u32__wrapping_sub(c, 2U * Q);
    c = core_num__u32__wrapping_add(c, Q & c >> 16U);
    c = core_num__u32__wrapping_add(c, Q & c >> 16U);
    pe_dst->data[i] = (uint16_t)c;
  }
}

/**
A monomorphic instance of symcrust.common.wipe_slice
with types uint32_t

*/
void symcrust_common_wipe_slice_df(Eurydice_slice pb_dst)
{
  uint8_t *uu____0 = (uint8_t *)core_slice___Slice_T___as_mut_ptr(pb_dst, uint32_t, uint32_t *);
  size_t uu____1 = Eurydice_slice_len(pb_dst, uint32_t);
  symcrust_common_wipe(uu____0, uu____1 * core_mem_size_of(uint32_t, size_t));
}

/**
A monomorphic instance of symcrust.common.wipe_slice
with types uint16_t

*/
void symcrust_common_wipe_slice_de(Eurydice_slice pb_dst)
{
  uint8_t *uu____0 = (uint8_t *)core_slice___Slice_T___as_mut_ptr(pb_dst, uint16_t, uint16_t *);
  size_t uu____1 = Eurydice_slice_len(pb_dst, uint16_t);
  symcrust_common_wipe(uu____0, uu____1 * core_mem_size_of(uint16_t, size_t));
}

static void
vector_mont_dot_product(
  Eurydice_slice pv_src1,
  Eurydice_slice pv_src2,
  Eurydice_arr_bc *pe_dst,
  Eurydice_arr_82 *pa_tmp
)
{
  size_t n_rows = Eurydice_slice_len(pv_src1, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pv_src2, Eurydice_arr_bc) == n_rows, "panic!");
  /* Zero pa_tmp and pe_dst */
  symcrust_common_wipe_slice_df(Eurydice_array_to_slice((size_t)256U, pa_tmp, uint32_t));
  symcrust_common_wipe_slice_de(Eurydice_array_to_slice((size_t)256U, pe_dst, uint16_t));
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    poly_element_mul_and_accumulate(&Eurydice_slice_index(pv_src1,
        i,
        Eurydice_arr_bc,
        Eurydice_arr_bc *),
      &Eurydice_slice_index(pv_src2, i, Eurydice_arr_bc, Eurydice_arr_bc *),
      pa_tmp);
  }
  /* write accumulator to dest and zero accumulator */
  /* write accumulator to dest and zero accumulator */
    montgomery_reduce_and_add_poly_element_accumulator_to_poly_element(pa_tmp,
      pe_dst)
  ;
}

static void poly_element_intt_layer_c(Eurydice_arr_bc *pe_src, size_t k, size_t len)
{
  for (size_t start = (size_t)0U; start < (size_t)256U; start = start + (size_t)2U * len)
  {
    uint32_t twiddle_factor = (uint32_t)ZETA_BIT_REV_TIMES_R.data[k];
    uint32_t twiddle_factor_mont = (uint32_t)ZETA_BIT_REV_TIMES_R_TIMES_NEG_Q_INV_MOD_R.data[k];
    k--;
    for (size_t j = (size_t)0U; j < len; j++)
    {
      uint32_t c0 = (uint32_t)pe_src->data[start + j];
      EURYDICE_ASSERT(c0 < Q, "panic!");
      uint32_t c1 = (uint32_t)pe_src->data[start + j + len];
      EURYDICE_ASSERT(c1 < Q, "panic!");
      uint32_t tmp = mod_add(c0, c1);
      c1 = mod_sub(c1, c0);
      c1 = mont_mul(c1, twiddle_factor, twiddle_factor_mont);
      pe_src->data[start + j] = (uint16_t)tmp;
      pe_src->data[start + j + len] = (uint16_t)c1;
    }
  }
}

static KRML_MUSTINLINE void
poly_element_intt_layer(Eurydice_arr_bc *pe_src, size_t k, size_t len)
{
  poly_element_intt_layer_c(pe_src, k, len);
}

#define INTT_FIXUP_TIMES_RSQR (1441U)

#define INTT_FIXUP_TIMES_RSQR_TIMES_NEQ_Q_INV_MOD_R (10079U)

static void poly_element_intt_and_mul_r(Eurydice_arr_bc *pe_src)
{
  poly_element_intt_layer(pe_src, (size_t)127U, (size_t)2U);
  poly_element_intt_layer(pe_src, (size_t)63U, (size_t)4U);
  poly_element_intt_layer(pe_src, (size_t)31U, (size_t)8U);
  poly_element_intt_layer(pe_src, (size_t)15U, (size_t)16U);
  poly_element_intt_layer(pe_src, (size_t)7U, (size_t)32U);
  poly_element_intt_layer(pe_src, (size_t)3U, (size_t)64U);
  poly_element_intt_layer(pe_src, (size_t)1U, (size_t)128U);
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t
    uu____0 =
      mont_mul((uint32_t)pe_src->data[i],
        INTT_FIXUP_TIMES_RSQR,
        INTT_FIXUP_TIMES_RSQR_TIMES_NEQ_Q_INV_MOD_R);
    pe_src->data[i] = (uint16_t)uu____0;
  }
}

static void
poly_element_sub(Eurydice_arr_bc *pe_src1, Eurydice_arr_bc *pe_src2, Eurydice_arr_bc *pe_dst)
{
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t uu____0 = mod_sub((uint32_t)pe_src1->data[i], (uint32_t)pe_src2->data[i]);
    pe_dst->data[i] = (uint16_t)uu____0;
  }
}

#define COMPRESS_MULCONSTANT (10321339U)

#define COMPRESS_SHIFTCONSTANT (35U)

static KRML_MUSTINLINE uint32_t
compress_coefficient(uint32_t n_bits_per_coefficient, uint32_t coefficient)
{
  /* When n_bits_per_coefficient < 12 we compress per Compress_d in draft FIPS 203 */
  if (n_bits_per_coefficient < 12U)
  {
    /* Multiply by 2^(n_bits_per_coefficient+1) / Q by multiplying by constant and shifting right */
    uint64_t multiplication = (uint64_t)coefficient * (uint64_t)COMPRESS_MULCONSTANT;
    coefficient =
      (uint32_t)(multiplication >>
        (uint32_t)(COMPRESS_SHIFTCONSTANT - (n_bits_per_coefficient + 1U)));
    coefficient++;
    /* final divide by two to get multiplication by 2^n_bits_per_coefficient / Q */
    coefficient = coefficient >> 1U;
    /* assert!(coefficient <= (1<<n_bits_per_coefficient));
      modular reduction by masking */
    coefficient = coefficient & ((1U << (uint32_t)n_bits_per_coefficient) - 1U);
  }
  return coefficient;
}

static KRML_MUSTINLINE void
encode_coefficient(
  uint32_t coefficient,
  uint32_t n_bits_in_coefficient,
  Eurydice_slice pb_dst,
  size_t *cb_dst_written,
  uint32_t *accumulator,
  uint32_t *n_bits_in_accumulator
)
{
  /* Note that the number of bits to encode is <= 12 while the accumulator has 32 bits,
    which means that if the accumulator is full, we only need to flush it once before
    encoding the remaining bits. */
  uint32_t n_bits_to_encode = min(n_bits_in_coefficient, 32U - n_bits_in_accumulator[0U]);
  uint32_t bits_to_encode = coefficient & ((1U << (uint32_t)n_bits_to_encode) - 1U);
  n_bits_in_coefficient = n_bits_in_coefficient - n_bits_to_encode;
  accumulator[0U] = accumulator[0U] | bits_to_encode << (uint32_t)n_bits_in_accumulator[0U];
  n_bits_in_accumulator[0U] = n_bits_in_accumulator[0U] + n_bits_to_encode;
  /* Flush the accumulator, if necessary */
  if (n_bits_in_accumulator[0U] == 32U)
  {
    Eurydice_slice
    uu____0 =
      Eurydice_slice_subslice3(pb_dst,
        cb_dst_written[0U],
        cb_dst_written[0U] + (size_t)4U,
        uint8_t *);
    /* original Rust expression is not an lvalue in C */
    Eurydice_arr_e9 lvalue = core_num__u32__to_le_bytes(accumulator[0U]);
    Eurydice_slice_copy(uu____0, Eurydice_array_to_slice((size_t)4U, &lvalue, uint8_t), uint8_t);
    cb_dst_written[0U] = cb_dst_written[0U] + (size_t)4U;
    /* Encode the remaining bits, if there are */
    accumulator[0U] = coefficient >> (uint32_t)n_bits_to_encode;
    n_bits_in_accumulator[0U] = n_bits_in_coefficient;
  }
}

static void
poly_element_compress_and_encode(
  Eurydice_arr_bc *pe_src,
  uint32_t n_bits_per_coefficient,
  Eurydice_slice pb_dst
)
{
  size_t cb_dst_written = (size_t)0U;
  uint32_t accumulator = 0U;
  uint32_t n_bits_in_accumulator = 0U;
  EURYDICE_ASSERT(n_bits_per_coefficient > 0U, "panic!");
  EURYDICE_ASSERT(n_bits_per_coefficient <= 12U, "panic!");
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t coefficient = (uint32_t)pe_src->data[i];
    EURYDICE_ASSERT(coefficient < Q, "panic!");
    /* First compress the coefficient.
      Encode the coefficient. */
    uint32_t coefficient0 = compress_coefficient(n_bits_per_coefficient, coefficient);
    encode_coefficient(coefficient0,
      n_bits_per_coefficient,
      pb_dst,
      &cb_dst_written,
      &accumulator,
      &n_bits_in_accumulator);
  }
  EURYDICE_ASSERT(n_bits_in_accumulator == 0U, "panic!");
}

static void
poly_element_sample_cbd_from_bytes(
  Eurydice_slice pb_src,
  uint32_t eta,
  Eurydice_arr_bc *pe_dst
)
{
  /* Note (Rust): using an index rather than incrementing pb_src in place. */
  size_t src_i = (size_t)0U;
  if (!(eta == 2U))
  {
    EURYDICE_ASSERT(eta == 3U, "panic!");
  }
  void *uu____0 = (void *)0U;
  if (!(eta == 3U))
  {
    for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i = i + (size_t)8U)
    {
      /* unconditionally load 4 bytes (32-bits -> 8 coefficients) into sample_bits */
      uint32_t
      sample_bits =
        core_num__u32__from_le_bytes(symcrust_ntt_slice_to_sub_array((size_t)4U,
            pb_src,
            src_i,
            Eurydice_arr_e9));
      src_i = src_i + (size_t)4U;
      /* sum bit samples - each consecutive slice of eta bits is summed together */
      sample_bits = (sample_bits & 1431655765U) + (sample_bits >> 1U & 1431655765U);
      KRML_MAYBE_FOR8(j,
        (size_t)0U,
        (size_t)8U,
        (size_t)1U,
        /* each coefficient is formed by taking the difference of two consecutive slices of eta bits
          the first eta bits are positive, the second eta bits are negative */
        uint32_t coefficient = sample_bits & 15U;
        sample_bits = sample_bits >> 4U;
        coefficient = core_num__u32__wrapping_sub(coefficient & 3U, coefficient >> 2U);
        if (!(coefficient >= (uint32_t)(int32_t)-2))
        {
          EURYDICE_ASSERT(coefficient <= 2U, "panic!");
        }
        coefficient = core_num__u32__wrapping_add(coefficient, Q & coefficient >> 16U);
        EURYDICE_ASSERT(coefficient < Q, "panic!");
        /* each coefficient is formed by taking the difference of two consecutive slices of eta bits
          the first eta bits are positive, the second eta bits are negative */
        pe_dst->data[i + j] = (uint16_t)coefficient;);
    }
    return;
  }
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i = i + (size_t)4U)
  {
    uint32_t
    sample_bits =
      core_num__u32__from_le_bytes(symcrust_ntt_slice_to_sub_array((size_t)4U,
          pb_src,
          src_i,
          Eurydice_arr_e9));
    src_i = src_i + (size_t)3U;
    sample_bits =
      (sample_bits & 2396745U) + (sample_bits >> 1U & 2396745U) + (sample_bits >> 2U & 2396745U);
    KRML_MAYBE_FOR4(j,
      (size_t)0U,
      (size_t)4U,
      (size_t)1U,
      /* each coefficient is formed by taking the difference of two consecutive slices of eta bits
        the first eta bits are positive, the second eta bits are negative */
      uint32_t coefficient = sample_bits & 63U;
      sample_bits = sample_bits >> 6U;
      coefficient = core_num__u32__wrapping_sub(coefficient & 3U, coefficient >> 3U);
      if (!(coefficient >= (uint32_t)(int32_t)-3))
      {
        EURYDICE_ASSERT(coefficient <= 3U, "panic!");
      }
      coefficient = core_num__u32__wrapping_add(coefficient, Q & coefficient >> 16U);
      EURYDICE_ASSERT(coefficient < Q, "panic!");
      pe_dst->data[i + j] = (uint16_t)coefficient;);
  }
}

static void vector_set_zero(Eurydice_slice pv_src)
{
  size_t n_rows = Eurydice_slice_len(pv_src, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    symcrust_common_wipe_slice_de(Eurydice_array_to_slice((size_t)256U,
        &Eurydice_slice_index(pv_src, i, Eurydice_arr_bc, Eurydice_arr_bc *),
        uint16_t));
  }
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice symcrust_key_atranspose_mut_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      (size_t)0U,
      m_len,
      Eurydice_arr_bc *);
}

static KRML_MUSTINLINE void
poly_element_mul_and_accumulate_aux(
  Eurydice_slice pm_src1,
  size_t n_rows,
  size_t i,
  size_t j,
  Eurydice_arr_bc *pe_src2,
  Eurydice_arr_82 *pa_tmp
)
{
  Eurydice_arr_bc
  *src1 = &Eurydice_slice_index(pm_src1, i * n_rows + j, Eurydice_arr_bc, Eurydice_arr_bc *);
  poly_element_mul_and_accumulate(src1, pe_src2, pa_tmp);
}

typedef struct _size_t__x2_s
{
  size_t *fst;
  size_t *snd;
}
_size_t__x2;

static void
matrix_vector_mont_mul_and_add(
  Eurydice_slice pm_src1,
  Eurydice_slice pv_src2,
  Eurydice_slice pv_dst,
  Eurydice_arr_82 *pa_tmp,
  uint8_t n_rows
)
{
  size_t n_rows0 = (size_t)n_rows;
  EURYDICE_ASSERT(n_rows0 > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows0 <= MATRIX_MAX_NROWS, "panic!");
  /* original Rust expression is not an lvalue in C */
  size_t lvalue = Eurydice_slice_len(pv_src2, Eurydice_arr_bc);
  _size_t__x2 uu____0 = { .fst = &lvalue, .snd = &n_rows0 };
  EURYDICE_ASSERT(uu____0.fst[0U] == uu____0.snd[0U], "panic!");
  /* original Rust expression is not an lvalue in C */
  size_t lvalue0 = Eurydice_slice_len(pv_dst, Eurydice_arr_bc);
  _size_t__x2 uu____1 = { .fst = &lvalue0, .snd = &n_rows0 };
  EURYDICE_ASSERT(uu____1.fst[0U] == uu____1.snd[0U], "panic!");
  symcrust_common_wipe_slice_df(Eurydice_array_to_slice((size_t)256U, pa_tmp, uint32_t));
  for (size_t i = (size_t)0U; i < n_rows0; i++)
  {
    for (size_t j = (size_t)0U; j < n_rows0; j++)
    {
      /* Zero pa_tmp */
      poly_element_mul_and_accumulate_aux(pm_src1,
        n_rows0,
        i,
        j,
        &Eurydice_slice_index(pv_src2, j, Eurydice_arr_bc, Eurydice_arr_bc *),
        pa_tmp);
    }
    /* write accumulator to dest and zero accumulator */
    montgomery_reduce_and_add_poly_element_accumulator_to_poly_element(pa_tmp,
      &Eurydice_slice_index(pv_dst, i, Eurydice_arr_bc, Eurydice_arr_bc *));
  }
}

static void vector_intt_and_mul_r(Eurydice_slice pv_src)
{
  size_t n_rows = Eurydice_slice_len(pv_src, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    poly_element_intt_and_mul_r(&Eurydice_slice_index(pv_src,
        i,
        Eurydice_arr_bc,
        Eurydice_arr_bc *));
  }
}

static void
poly_element_add(Eurydice_arr_bc *pe_src1, Eurydice_arr_bc *pe_src2, Eurydice_arr_bc *pe_dst)
{
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t uu____0 = mod_add((uint32_t)pe_src1->data[i], (uint32_t)pe_src2->data[i]);
    pe_dst->data[i] = (uint16_t)uu____0;
  }
}

static void
vector_compress_and_encode(
  Eurydice_slice pv_src,
  uint32_t n_bits_per_coefficient,
  Eurydice_slice pb_dst
)
{
  size_t n_rows = Eurydice_slice_len(pv_src, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(n_bits_per_coefficient > 0U, "panic!");
  EURYDICE_ASSERT(n_bits_per_coefficient <= 12U, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pb_dst, uint8_t) ==
      n_rows *
        (size_t)(n_bits_per_coefficient * ((uint32_t)SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / 8U)),
    "panic!");
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    /* Note (Rust): had to change this to do range computation as opposed to in-place pointer
      increment */
    size_t
    pb_dst_index =
      i * (size_t)n_bits_per_coefficient * (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
    poly_element_compress_and_encode(&Eurydice_slice_index(pv_src,
        i,
        Eurydice_arr_bc,
        Eurydice_arr_bc *),
      n_bits_per_coefficient,
      Eurydice_slice_subslice_from(pb_dst, pb_dst_index, uint8_t, size_t, Eurydice_derefed_slice));
  }
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice symcrust_key_t_mut_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      m_len,
      m_len + self.ptr->header.n_rows,
      Eurydice_arr_bc *);
}

/**
This function found in impl {core::result::Result<T, E>}
*/
/**
A monomorphic instance of core.result.unwrap_41
with types  Eurydice_arr uint8_t[[$64size_t]]*, core_array_TryFromSliceError

*/
static Eurydice_arr_06 *unwrap_41_f2(core_result_Result_20 self)
{
  if (self.tag == core_result_Ok)
  {
    return self.val.case_Ok;
  }
  else
  {
    KRML_HOST_EPRINTF("KaRaMeL abort at %s:%d\n%s\n", __FILE__, __LINE__, "unwrap not Ok");
    KRML_HOST_EXIT(255U);
  }
}

symcrust_common_Error
symcrust_mlkem_encapsulate_internal(
  Eurydice_dst_6f pk_mlkem_key,
  Eurydice_slice pb_agreed_secret,
  Eurydice_slice pb_ciphertext,
  Eurydice_arr_60 *pb_random,
  symcrust_ntt_InternalComputationTemporaries *p_comp_temps
)
{
  size_t cb_agreed_secret = Eurydice_slice_len(pb_agreed_secret, uint8_t);
  size_t cb_ciphertext = Eurydice_slice_len(pb_ciphertext, uint8_t);
  Eurydice_arr_73 cbd_sample_buffer = { .data = { 0U } };
  /* ERROR sc_error = NO_ERROR;
    PVECTOR pvr_inner;
    PVECTOR pv_tmp;
    PPOLYELEMENT pe_tmp0, pe_tmp1;
    PPOLYELEMENT_ACCUMULATOR pa_tmp;
    PSHA3_512_STATE pHashState = &p_comp_temps->hash_state0.sha3_512State;
    PSHAKE256_STATE pShakeBaseState = &p_comp_temps->hash_state0.shake256State;
    PSHAKE256_STATE pShakeWorkState = &p_comp_temps->hash_state1.shake256State;
    SIZE_T cb_u, cb_v;
    UINT32 i; */
  uint8_t n_rows = pk_mlkem_key.ptr->header.params.n_rows;
  uint8_t n_bits_of_u = pk_mlkem_key.ptr->header.params.n_bits_of_u;
  uint8_t n_bits_of_v = pk_mlkem_key.ptr->header.params.n_bits_of_v;
  uint8_t n_eta1 = pk_mlkem_key.ptr->header.params.n_eta1;
  uint8_t n_eta2 = pk_mlkem_key.ptr->header.params.n_eta2;
  /* let cbPolyElement = pk_mlkem_key->params.cbPolyElement;
    let cb_vector = pk_mlkem_key->params.cb_vector;
    u vector encoded with n_bits_of_u * MLWE_POLYNOMIAL_COEFFICIENTS bits per polynomial */
  size_t
  cb_u =
    (size_t)n_rows * (size_t)n_bits_of_u * (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
  /* v polynomial encoded with n_bits_of_v * MLWE_POLYNOMIAL_COEFFICIENTS bits */
  size_t cb_v = (size_t)n_bits_of_v * (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
  if (!(cb_agreed_secret != SYMCRUST_MLKEM_SIZEOF_AGREED_SECRET))
  {
    if (!(cb_ciphertext != cb_u + cb_v))
    {
      Eurydice_slice
      pvr_inner =
        Eurydice_array_to_subslice3(&p_comp_temps->ab_vector_buffer0,
          (size_t)0U,
          (size_t)n_rows,
          Eurydice_arr_bc *);
      Eurydice_slice
      pv_tmp =
        Eurydice_array_to_subslice3(&p_comp_temps->ab_vector_buffer1,
          (size_t)0U,
          (size_t)n_rows,
          Eurydice_arr_bc *);
      Eurydice_arr_bc *pe_tmp0 = &p_comp_temps->ab_poly_element_buffer0;
      Eurydice_arr_bc *pe_tmp1 = &p_comp_temps->ab_poly_element_buffer1;
      Eurydice_arr_82 *pa_tmp = &p_comp_temps->ab_poly_element_accumulator_buffer;
      /* cbd_sample_buffer = (K || rOuter) = SHA3-512(pb_random || encapsKeyHash) */
      symcrust_hash_sha3_512_init(&p_comp_temps->hash_state0);
      symcrust_hash_sha3_512_append(&p_comp_temps->hash_state0,
        Eurydice_array_to_slice((size_t)32U, pb_random, uint8_t));
      symcrust_hash_sha3_512_append(&p_comp_temps->hash_state0,
        Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.encaps_key_hash, uint8_t));
      /* pv_tmp = u = INTT(Atranspose o rInner) + e1
        Compress and encode u into prefix of ciphertext
        pe_tmp0 = (t o r) ./ R
        pe_tmp0 = INTT(t o r)
        Expand e2 polynomial in pe_tmp1
        pe_tmp1 = mu
        FIXME (same as above)
        Compress and encode v into remainder of ciphertext */
      symcrust_hash_HashState *uu____0 = &p_comp_temps->hash_state0;
      symcrust_hash_sha3_512_result(uu____0,
        unwrap_41_f2(Eurydice_slice_to_ref_array((size_t)64U,
            Eurydice_array_to_subslice3(&cbd_sample_buffer,
              (size_t)0U,
              SYMCRUST_HASH_SHA3_512_RESULT_SIZE,
              uint8_t *),
            Eurydice_slice,
            Eurydice_arr_06 *,
            core_array_TryFromSliceError,
            core_result_Result_20)));
      Eurydice_slice_copy(Eurydice_slice_subslice3(pb_agreed_secret,
          (size_t)0U,
          SYMCRUST_MLKEM_SIZEOF_AGREED_SECRET,
          uint8_t *),
        Eurydice_array_to_subslice3(&cbd_sample_buffer,
          (size_t)0U,
          SYMCRUST_MLKEM_SIZEOF_AGREED_SECRET,
          uint8_t *),
        uint8_t);
      symcrust_hash_shake256_init(&p_comp_temps->hash_state0);
      symcrust_hash_shake256_append(&p_comp_temps->hash_state0,
        Eurydice_array_to_subslice3(&cbd_sample_buffer,
          cb_agreed_secret,
          cb_agreed_secret + (size_t)32U,
          uint8_t *));
      for (uint8_t i = 0U; i < n_rows; i = (uint32_t)i + 1U)
      {
        /* Note (Rust): should we have a type that is less strict for the output of sha3_512_result?
          Note (Rust): no debug_assert!(SIZEOF_AGREED_SECRET < SHA3_512_RESULT_SIZE)?
          Write K to pb_agreed_secret
          Initialize p_shake_stateBase with rOuter */
        cbd_sample_buffer.data[0U] = i;
        symcrust_hash_shake256_state_copy(&p_comp_temps->hash_state0, &p_comp_temps->hash_state1);
        symcrust_hash_shake256_append(&p_comp_temps->hash_state1,
          Eurydice_array_to_subslice3(&cbd_sample_buffer, (size_t)0U, (size_t)1U, uint8_t *));
        symcrust_hash_shake256_extract(&p_comp_temps->hash_state1,
          Eurydice_array_to_subslice3(&cbd_sample_buffer,
            (size_t)0U,
            (size_t)64U * (size_t)n_eta1,
            uint8_t *),
          false);
        poly_element_sample_cbd_from_bytes(Eurydice_array_to_slice((size_t)193U,
            &cbd_sample_buffer,
            uint8_t),
          (uint32_t)n_eta1,
          &Eurydice_slice_index(pvr_inner, (size_t)i, Eurydice_arr_bc, Eurydice_arr_bc *));
      }
      vector_ntt(pvr_inner);
      vector_set_zero(pv_tmp);
      matrix_vector_mont_mul_and_add(symcrust_key_atranspose_mut_fd(pk_mlkem_key),
        pvr_inner,
        pv_tmp,
        pa_tmp,
        n_rows);
      vector_intt_and_mul_r(pv_tmp);
      for (uint8_t i = 0U; i < n_rows; i = (uint32_t)i + 1U)
      {
        /* Perform NTT on rInner
          Set pv_tmp to 0
          TODO: write a helper function -- any way to do this better?
          pv_tmp.copy_from_slice(&vec![POLYELEMENT_ZERO; n_rows as usize].into_boxed_slice());
          VectorSetZero( pv_tmp );
          pv_tmp = (Atranspose o rInner) ./ R
          pv_tmp = INTT(Atranspose o rInner) */
        cbd_sample_buffer.data[0U] = (uint32_t)n_rows + (uint32_t)i;
        symcrust_hash_shake256_state_copy(&p_comp_temps->hash_state0, &p_comp_temps->hash_state1);
        symcrust_hash_shake256_append(&p_comp_temps->hash_state1,
          Eurydice_array_to_subslice3(&cbd_sample_buffer, (size_t)0U, (size_t)1U, uint8_t *));
        symcrust_hash_shake256_extract(&p_comp_temps->hash_state1,
          Eurydice_array_to_subslice3(&cbd_sample_buffer,
            (size_t)0U,
            (size_t)64U * (size_t)n_eta2,
            uint8_t *),
          false);
        poly_element_sample_cbd_from_bytes(Eurydice_array_to_slice((size_t)193U,
            &cbd_sample_buffer,
            uint8_t),
          (uint32_t)n_eta2,
          pe_tmp0);
        /* Note (Rust): in-place operation here, was:
          PolyElementAdd( INTERNAL_MLKEM_VECTOR_ELEMENT(i, pv_tmp), pe_tmp0, INTERNAL_MLKEM_VECTOR_ELEMENT(i, pv_tmp) );
          Added a copy -- TODO: measure performance impact of the copy */
        Eurydice_arr_bc
        copy = Eurydice_slice_index(pv_tmp, (size_t)i, Eurydice_arr_bc, Eurydice_arr_bc *);
        poly_element_add(&copy,
          pe_tmp0,
          &Eurydice_slice_index(pv_tmp, (size_t)i, Eurydice_arr_bc, Eurydice_arr_bc *));
      }
      vector_compress_and_encode(pv_tmp,
        (uint32_t)n_bits_of_u,
        Eurydice_slice_subslice3(pb_ciphertext, (size_t)0U, cb_u, uint8_t *));
      vector_mont_dot_product(symcrust_key_t_mut_fd(pk_mlkem_key), pvr_inner, pe_tmp0, pa_tmp);
      poly_element_intt_and_mul_r(pe_tmp0);
      cbd_sample_buffer.data[0U] = 2U * (uint32_t)n_rows;
      symcrust_hash_shake256_state_copy(&p_comp_temps->hash_state0, &p_comp_temps->hash_state1);
      symcrust_hash_shake256_append(&p_comp_temps->hash_state1,
        Eurydice_array_to_subslice3(&cbd_sample_buffer, (size_t)0U, (size_t)1U, uint8_t *));
      symcrust_hash_shake256_extract(&p_comp_temps->hash_state1,
        Eurydice_array_to_subslice3(&cbd_sample_buffer,
          (size_t)0U,
          (size_t)64U * (size_t)n_eta2,
          uint8_t *),
        false);
      poly_element_sample_cbd_from_bytes(Eurydice_array_to_slice((size_t)193U,
          &cbd_sample_buffer,
          uint8_t),
        (uint32_t)n_eta2,
        pe_tmp1);
      /* peTmp = INTT(t o r) + e2
        Note (Rust): in-place operation, was:
        PolyElementAdd( pe_tmp0, pe_tmp1, pe_tmp0 );
        FIXME (measure performance issues, adjust) */
      Eurydice_arr_bc copy = pe_tmp0[0U];
      poly_element_add(&copy, pe_tmp1, pe_tmp0);
      poly_element_decode_and_decompress(Eurydice_array_to_slice((size_t)32U, pb_random, uint8_t),
        1U,
        pe_tmp1);
      /* pe_tmp0 = v = INTT(t o r) + e2 + mu */
      Eurydice_arr_bc copy0 = pe_tmp0[0U];
      poly_element_add(&copy0, pe_tmp1, pe_tmp0);
      poly_element_compress_and_encode(pe_tmp0,
        (uint32_t)n_bits_of_v,
        Eurydice_slice_subslice_from(pb_ciphertext, cb_u, uint8_t, size_t, Eurydice_derefed_slice));
      return symcrust_common_Error_NoError;
    }
  }
  return symcrust_common_Error_InvalidArgument;
}

/**
A monomorphic instance of core.result.Result
with types Eurydice_vec uint8_t, alloc_collections_TryReserveError

*/
typedef struct Result_49_s
{
  core_result_Result_10 tag;
  union {
    Eurydice_vec case_Ok;
    alloc_collections_TryReserveErrorKind case_Err;
  }
  val;
}
Result_49;

/**
This function found in impl {alloc::vec::Vec<T>}
*/
/**
A monomorphic instance of alloc.vec.try_with_capacity_5d
with types uint8_t

*/
static Result_49 try_with_capacity_5d_90(size_t len)
{
  if (Eurydice_vec_overflows(len, uint8_t, bool))
  {
    return
      (
        KRML_CLITERAL(Result_49){
          .tag = core_result_Err,
          .val = { .case_Err = { .tag = alloc_collections_CapacityOverflow } }
        }
      );
  }
  else
  {
    Eurydice_vec v = Eurydice_vec_alloc(len, uint8_t, Eurydice_vec);
    if (Eurydice_vec_failed(v, uint8_t, bool))
    {
      return
        (
          KRML_CLITERAL(Result_49){
            .tag = core_result_Err,
            .val = {
              .case_Err = {
                .tag = alloc_collections_AllocError,
                .layout = Eurydice_layout(uint8_t, core_alloc_layout_Layout)
              }
            }
          }
        );
    }
    else
    {
      return (KRML_CLITERAL(Result_49){ .tag = core_result_Ok, .val = { .case_Ok = v } });
    }
  }
}

symcrust_common_Error
symcrust_mlkem_decapsulate(
  Eurydice_dst_6f pk_mlkem_key,
  Eurydice_slice pb_ciphertext,
  Eurydice_slice pb_agreed_secret
)
{
  size_t cb_ciphertext = Eurydice_slice_len(pb_ciphertext, uint8_t);
  size_t cb_agreed_secret = Eurydice_slice_len(pb_agreed_secret, uint8_t);
  symcrust_ntt_InternalComputationTemporaries lit;
  Eurydice_arr_bc repeat_expression[4U];
  KRML_MAYBE_FOR4(i,
    (size_t)0U,
    (size_t)4U,
    (size_t)1U,
    repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
  memcpy(lit.ab_vector_buffer0.data, repeat_expression, (size_t)4U * sizeof (Eurydice_arr_bc));
  Eurydice_arr_bc repeat_expression0[4U];
  KRML_MAYBE_FOR4(i,
    (size_t)0U,
    (size_t)4U,
    (size_t)1U,
    repeat_expression0[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
  memcpy(lit.ab_vector_buffer1.data, repeat_expression0, (size_t)4U * sizeof (Eurydice_arr_bc));
  lit.ab_poly_element_buffer0 = SYMCRUST_KEY_POLYELEMENT_ZERO;
  lit.ab_poly_element_buffer1 = SYMCRUST_KEY_POLYELEMENT_ZERO;
  memset(lit.ab_poly_element_accumulator_buffer.data, 0U, 256U * sizeof (uint32_t));
  lit.hash_state0 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
  lit.hash_state1 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
  core_result_Result_80
  p_comp_temps0 =
    alloc_boxed__alloc__boxed__Box_T___try_new(lit,
      symcrust_ntt_InternalComputationTemporaries,
      core_result_Result_80);
  symcrust_common_Error uu____0;
  if (p_comp_temps0.tag == core_result_Ok)
  {
    symcrust_ntt_InternalComputationTemporaries *p_comp_temps = p_comp_temps0.f0;
    symcrust_ntt_InternalComputationTemporaries *p_comp_temps1 = p_comp_temps;
    Eurydice_arr_60 pb_decrypted_random = { .data = { 0U } };
    Eurydice_arr_60 pb_decapsulated_secret = { .data = { 0U } };
    Eurydice_arr_60 pb_implicit_rejection_secret = { .data = { 0U } };
    /* PBYTE pbReadCiphertext, pbReencapsulatedCiphertext;
      BOOLEAN successfulReencrypt;
      Note (Rust): we originally perform a single call to malloc() and use the first few bytes for
      the temporary computations, then for the two temporary ciphertexts. Rust does not allow to
      do this, so we perform two allocations.
      Note (Rust): rather than use the (simple) solution below, which does not allow catching
      memory allocation failures, we instead use the experimental try_with_capacity API:
      let pb_comp_ciphers = vec![0u8; 2*cb_ciphertext].into_boxed_slice(); */
    Result_49 pb_comp_ciphers = try_with_capacity_5d_90((size_t)2U * cb_ciphertext);
    if (pb_comp_ciphers.tag == core_result_Ok)
    {
      Eurydice_vec pb_comp_ciphers0 = pb_comp_ciphers.val.case_Ok;
      Eurydice_vec pb_comp_ciphers1 = pb_comp_ciphers0;
      alloc_vec__alloc__vec__Vec_T___resize(&pb_comp_ciphers1,
        (size_t)2U * cb_ciphertext,
        0U,
        uint8_t,
        void *,
        void *);
      Eurydice_slice
      pb_comp_ciphers2 =
        alloc_vec__alloc__vec__Vec_T___into_boxed_slice(pb_comp_ciphers1,
          uint8_t,
          void *,
          Eurydice_slice);
      Eurydice_slice_uint8_t_x2
      uu____1 =
        Eurydice_slice_split_at_mut(pb_comp_ciphers2,
          cb_ciphertext,
          uint8_t,
          Eurydice_slice_uint8_t_x2);
      Eurydice_slice pb_read_ciphertext = uu____1.fst;
      Eurydice_slice pb_reencapsulated_ciphertext = uu____1.snd;
      /*     ERROR sc_error = NO_ERROR;
            SIZE_T cb_u, cb_v, cbCopy;
            PVECTOR pvu;
            PPOLYELEMENT pe_tmp0, pe_tmp1;
            PPOLYELEMENT_ACCUMULATOR pa_tmp;
            PSHAKE256_STATE p_shake_state; */
      uint8_t n_rows = pk_mlkem_key.ptr->header.params.n_rows;
      uint8_t n_bits_of_u = pk_mlkem_key.ptr->header.params.n_bits_of_u;
      uint8_t n_bits_of_v = pk_mlkem_key.ptr->header.params.n_bits_of_v;
      size_t
      cb_u =
        (size_t)n_rows * (size_t)n_bits_of_u *
          (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
      size_t cb_v = (size_t)n_bits_of_v * (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
      if (!(cb_agreed_secret != SYMCRUST_MLKEM_SIZEOF_AGREED_SECRET))
      {
        if (!(cb_ciphertext != cb_u + cb_v))
        {
          if (pk_mlkem_key.ptr->header.has_private_key)
          {
            /* let cbPolyElement = pk_mlkem_key.params.cbPolyElement;
              let cb_vector = pk_mlkem_key.params.cb_vector;
              u vector encoded with n_bits_of_u * MLWE_POLYNOMIAL_COEFFICIENTS bits per polynomial
              v polynomial encoded with n_bits_of_v * MLWE_POLYNOMIAL_COEFFICIENTS bits
              Read the input ciphertext once to local pbReadCiphertext to ensure our view of ciphertext consistent */
            Eurydice_slice_copy(pb_read_ciphertext, pb_ciphertext, uint8_t);
            /* Decode and decompress u
              Perform NTT on u
              pe_tmp0 = (s o NTT(u)) ./ R
              pe_tmp0 = INTT(s o NTT(u))
              Decode and decompress v
              pbDecryptedRandom = m' = Encoding of w
              Compute:
               pbDecapsulatedSecret = K' = Decapsulated secret (without implicit rejection)
               pbReencapsulatedCiphertext = c' = Ciphertext from re-encapsulating decrypted random value
              Constant time test if re-encryption successful
              If not successful, perform side-channel-safe copy of Implicit Rejection secret over Decapsulated secret
              FIXME, was:
              SymCryptScsCopy( pbImplicitRejectionSecret, cbCopy, pbDecapsulatedSecret, SIZEOF_AGREED_SECRET );
              Write agreed secret (with implicit rejection) to pb_agreed_secret */
            Eurydice_slice
            pvu =
              Eurydice_array_to_subslice3(&p_comp_temps1->ab_vector_buffer0,
                (size_t)0U,
                (size_t)n_rows,
                Eurydice_arr_bc *);
            Eurydice_arr_bc *pe_tmp0 = &p_comp_temps1->ab_poly_element_buffer0;
            Eurydice_arr_bc *pe_tmp1 = &p_comp_temps1->ab_poly_element_buffer1;
            Eurydice_arr_82 *pa_tmp = &p_comp_temps1->ab_poly_element_accumulator_buffer;
            symcrust_common_Error
            sc_error =
              vector_decode_and_decompress(Eurydice_slice_subslice3(pb_read_ciphertext,
                  (size_t)0U,
                  cb_u,
                  uint8_t *),
                (uint32_t)n_bits_of_u,
                pvu);
            /* original Rust expression is not an lvalue in C */
            symcrust_common_Error lvalue0 = symcrust_common_Error_NoError;
            EURYDICE_ASSERT(symcrust_common_eq_41(&sc_error, &lvalue0), "panic!");
            vector_ntt(pvu);
            vector_mont_dot_product(symcrust_key_s_mut_fd(pk_mlkem_key), pvu, pe_tmp0, pa_tmp);
            poly_element_intt_and_mul_r(pe_tmp0);
            symcrust_common_Error
            sc_error0 =
              poly_element_decode_and_decompress(Eurydice_slice_subslice_from(pb_read_ciphertext,
                  cb_u,
                  uint8_t,
                  size_t,
                  Eurydice_derefed_slice),
                (uint32_t)n_bits_of_v,
                pe_tmp1);
            /* original Rust expression is not an lvalue in C */
            symcrust_common_Error lvalue1 = symcrust_common_Error_NoError;
            EURYDICE_ASSERT(symcrust_common_eq_41(&sc_error0, &lvalue1), "panic!");
            /* pe_tmp0 = w = v - INTT(s o NTT(u))
              FIXME */
            Eurydice_arr_bc copy = pe_tmp0[0U];
            poly_element_sub(pe_tmp1, &copy, pe_tmp0);
            poly_element_compress_and_encode(pe_tmp0,
              1U,
              Eurydice_array_to_slice((size_t)32U, &pb_decrypted_random, uint8_t));
            symcrust_common_Error
            sc_error1 =
              symcrust_mlkem_encapsulate_internal(pk_mlkem_key,
                Eurydice_array_to_slice((size_t)32U, &pb_decapsulated_secret, uint8_t),
                pb_reencapsulated_ciphertext,
                &pb_decrypted_random,
                p_comp_temps1);
            /* original Rust expression is not an lvalue in C */
            symcrust_common_Error lvalue = symcrust_common_Error_NoError;
            EURYDICE_ASSERT(symcrust_common_eq_41(&sc_error1, &lvalue), "panic!");
            /* Compute the secret we will return if using implicit rejection
              pbImplicitRejectionSecret = K_bar = SHAKE256( z || c ) */
            symcrust_hash_HashState *p_shake_state = &p_comp_temps1->hash_state0;
            symcrust_hash_shake256_init(p_shake_state);
            symcrust_hash_shake256_append(p_shake_state,
              Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.private_random,
                uint8_t));
            symcrust_hash_shake256_append(p_shake_state, pb_read_ciphertext);
            symcrust_hash_shake256_extract(p_shake_state,
              Eurydice_array_to_slice((size_t)32U, &pb_implicit_rejection_secret, uint8_t),
              false);
            bool
            successful_reencrypt =
              Eurydice_slice_eq(&pb_reencapsulated_ciphertext,
                &pb_read_ciphertext,
                uint8_t,
                bool);
            size_t
            cb_copy =
              core_num__usize__wrapping_sub((size_t)successful_reencrypt, (size_t)1U) &
                SYMCRUST_MLKEM_SIZEOF_AGREED_SECRET;
            Eurydice_slice_copy(Eurydice_array_to_subslice3(&pb_decapsulated_secret,
                (size_t)0U,
                cb_copy,
                uint8_t *),
              Eurydice_array_to_subslice3(&pb_implicit_rejection_secret,
                (size_t)0U,
                cb_copy,
                uint8_t *),
              uint8_t);
            Eurydice_slice_copy(pb_agreed_secret,
              Eurydice_array_to_slice((size_t)32U, &pb_decapsulated_secret, uint8_t),
              uint8_t);
            return symcrust_common_Error_NoError;
          }
        }
      }
      uu____0 = symcrust_common_Error_InvalidArgument;
    }
    else
    {
      uu____0 = symcrust_common_Error_MemoryAllocationFailure;
    }
  }
  else
  {
    uu____0 = symcrust_common_Error_MemoryAllocationFailure;
  }
  return uu____0;
}

/**
This function found in impl {core::result::Result<T, E>}
*/
/**
A monomorphic instance of core.result.unwrap_41
with types  Eurydice_arr uint8_t[[$32size_t]]*, core_array_TryFromSliceError

*/
static Eurydice_arr_60 *unwrap_41_2e(core_result_Result_30 self)
{
  if (self.tag == core_result_Ok)
  {
    return self.val.case_Ok;
  }
  else
  {
    KRML_HOST_EPRINTF("KaRaMeL abort at %s:%d\n%s\n", __FILE__, __LINE__, "unwrap not Ok");
    KRML_HOST_EXIT(255U);
  }
}

symcrust_common_Error
symcrust_mlkem_encapsulate_ex(
  Eurydice_dst_6f pk_mlkem_key,
  Eurydice_slice pb_random,
  Eurydice_slice pb_agreed_secret,
  Eurydice_slice pb_ciphertext
)
{
  /* wrapper enforce it */
  size_t cb_random = Eurydice_slice_len(pb_random, uint8_t);
  symcrust_common_Error uu____0;
  /* let cb_agreed_secret = pb_agreed_secret.len();
    let cb_ciphertext = pb_ciphertext.len(); */
  if (cb_random != SYMCRUST_MLKEM_SIZEOF_ENCAPS_RANDOM)
  {
    uu____0 = symcrust_common_Error_InvalidArgument;
  }
  else
  {
    symcrust_ntt_InternalComputationTemporaries lit;
    Eurydice_arr_bc repeat_expression[4U];
    KRML_MAYBE_FOR4(i,
      (size_t)0U,
      (size_t)4U,
      (size_t)1U,
      repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
    memcpy(lit.ab_vector_buffer0.data, repeat_expression, (size_t)4U * sizeof (Eurydice_arr_bc));
    Eurydice_arr_bc repeat_expression0[4U];
    KRML_MAYBE_FOR4(i,
      (size_t)0U,
      (size_t)4U,
      (size_t)1U,
      repeat_expression0[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
    memcpy(lit.ab_vector_buffer1.data, repeat_expression0, (size_t)4U * sizeof (Eurydice_arr_bc));
    lit.ab_poly_element_buffer0 = SYMCRUST_KEY_POLYELEMENT_ZERO;
    lit.ab_poly_element_buffer1 = SYMCRUST_KEY_POLYELEMENT_ZERO;
    memset(lit.ab_poly_element_accumulator_buffer.data, 0U, 256U * sizeof (uint32_t));
    lit.hash_state0 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
    lit.hash_state1 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
    core_result_Result_80
    p_comp_temps =
      alloc_boxed__alloc__boxed__Box_T___try_new(lit,
        symcrust_ntt_InternalComputationTemporaries,
        core_result_Result_80);
    if (p_comp_temps.tag == core_result_Ok)
    {
      symcrust_ntt_InternalComputationTemporaries *p_comp_temps0 = p_comp_temps.f0;
      symcrust_ntt_InternalComputationTemporaries *p_comp_temps1 = p_comp_temps0;
      Eurydice_dst_6f uu____1 = pk_mlkem_key;
      Eurydice_slice uu____2 = pb_agreed_secret;
      Eurydice_slice uu____3 = pb_ciphertext;
      Eurydice_arr_60
      *uu____4 =
        unwrap_41_2e(Eurydice_slice_to_ref_array((size_t)32U,
            pb_random,
            Eurydice_slice,
            Eurydice_arr_60 *,
            core_array_TryFromSliceError,
            core_result_Result_30));
      return symcrust_mlkem_encapsulate_internal(uu____1, uu____2, uu____3, uu____4, p_comp_temps1);
    }
    else
    {
      uu____0 = symcrust_common_Error_MemoryAllocationFailure;
    }
  }
  return uu____0;
}

symcrust_common_Error
symcrust_mlkem_encapsulate(
  Eurydice_dst_6f pk_mlkem_key,
  Eurydice_slice pb_agreed_secret,
  Eurydice_slice pb_ciphertext
)
{
  Eurydice_arr_60 pbm = { .data = { 0U } };
  symcrust_common_Error
  sc_error = symcrust_common_random(Eurydice_array_to_slice((size_t)32U, &pbm, uint8_t));
  /* original Rust expression is not an lvalue in C */
  symcrust_common_Error lvalue = symcrust_common_Error_NoError;
  if
  (
    !symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error,
      &lvalue)
  )
  {
    return
      symcrust_mlkem_encapsulate_ex(pk_mlkem_key,
        Eurydice_array_to_slice((size_t)32U, &pbm, uint8_t),
        pb_agreed_secret,
        pb_ciphertext);
  }
  return sc_error;
}

symcrust_key_InternalParams
symcrust_key_get_internal_params_from_params(symcrust_key_Params params)
{
  switch (params)
  {
    case symcrust_key_Params_MlKem512:
      {
        break;
      }
    case symcrust_key_Params_MlKem768:
      {
        return SYMCRUST_KEY_INTERNAL_PARAMS_ML_KEM768;
      }
    case symcrust_key_Params_MlKem1024:
      {
        return SYMCRUST_KEY_INTERNAL_PARAMS_ML_KEM1024;
      }
    default:
      {
        KRML_HOST_EPRINTF("KaRaMeL incomplete match at %s:%d\n", __FILE__, __LINE__);
        KRML_HOST_EXIT(253U);
      }
  }
  return SYMCRUST_KEY_INTERNAL_PARAMS_ML_KEM512;
}

size_t symcrust_mlkem_sizeof_ciphertext_from_params(symcrust_key_Params params)
{
  symcrust_key_InternalParams
  internal_params = symcrust_key_get_internal_params_from_params(params);
  /* u vector encoded with n_bits_of_u * MLWE_POLYNOMIAL_COEFFICIENTS bits per polynomial */
  size_t
  cb_u =
    (size_t)internal_params.n_rows * (size_t)internal_params.n_bits_of_u *
      (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
  /* v polynomial encoded with n_bits_of_v * MLWE_POLYNOMIAL_COEFFICIENTS bits */
  size_t
  cb_v =
    (size_t)internal_params.n_bits_of_v * (SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
  /* original Rust expression is not an lvalue in C */
  symcrust_key_Params lvalue = symcrust_key_Params_MlKem512;
  if
  (
    !symcrust_key__core__cmp__PartialEq_symcrust__key__Params__for_symcrust__key__Params__ne(&internal_params.params,
      &lvalue)
  )
  {
    EURYDICE_ASSERT(cb_u + cb_v == SYMCRUST_MLKEM_CIPHERTEXT_SIZE_MLKEM512, "panic!");
  }
  /* original Rust expression is not an lvalue in C */
  symcrust_key_Params lvalue0 = symcrust_key_Params_MlKem768;
  if
  (
    !symcrust_key__core__cmp__PartialEq_symcrust__key__Params__for_symcrust__key__Params__ne(&internal_params.params,
      &lvalue0)
  )
  {
    EURYDICE_ASSERT(cb_u + cb_v == SYMCRUST_MLKEM_CIPHERTEXT_SIZE_MLKEM768, "panic!");
  }
  /* original Rust expression is not an lvalue in C */
  symcrust_key_Params lvalue1 = symcrust_key_Params_MlKem1024;
  if
  (
    !symcrust_key__core__cmp__PartialEq_symcrust__key__Params__for_symcrust__key__Params__ne(&internal_params.params,
      &lvalue1)
  )
  {
    EURYDICE_ASSERT(cb_u + cb_v == SYMCRUST_MLKEM_CIPHERTEXT_SIZE_MLKEM1024, "panic!");
  }
  return cb_u + cb_v;
}

size_t symcrust_mlkem_sizeof_encoded_uncompressed_vector(size_t _n_rows)
{
  return (size_t)384U * _n_rows;
}

size_t symcrust_mlkem_sizeof_format_decapsulation_key(size_t _n_rows)
{
  return
    (size_t)2U * symcrust_mlkem_sizeof_encoded_uncompressed_vector(_n_rows) +
      (size_t)3U * (size_t)32U;
}

size_t symcrust_mlkem_sizeof_format_encapsulation_key(size_t _n_rows)
{
  return symcrust_mlkem_sizeof_encoded_uncompressed_vector(_n_rows) + (size_t)32U;
}

size_t
symcrust_mlkem_sizeof_key_format_from_params(
  symcrust_key_Params params,
  symcrust_key_Format format
)
{
  symcrust_key_InternalParams
  internal_params = symcrust_key_get_internal_params_from_params(params);
  size_t uu____0;
  switch (format)
  {
    case symcrust_key_Format_PrivateSeed:
      {
        uu____0 = SYMCRUST_MLKEM_SIZEOF_FORMAT_PRIVATE_SEED;
        break;
      }
    case symcrust_key_Format_DecapsulationKey:
      {
        uu____0 = symcrust_mlkem_sizeof_format_decapsulation_key((size_t)internal_params.n_rows);
        break;
      }
    case symcrust_key_Format_EncapsulationKey:
      {
        uu____0 = symcrust_mlkem_sizeof_format_encapsulation_key((size_t)internal_params.n_rows);
        break;
      }
    default:
      {
        KRML_HOST_EPRINTF("KaRaMeL incomplete match at %s:%d\n", __FILE__, __LINE__);
        KRML_HOST_EXIT(253U);
      }
  }
  return uu____0;
}

core_result_Result_67 symcrust_key_key_allocate2(symcrust_key_Params params)
{
  symcrust_key_InternalParams
  internal_params = symcrust_key_get_internal_params_from_params(params);
  size_t n_rows = (size_t)internal_params.n_rows;
  symcrust_key_PreKey2Header
  header =
    {
      .algorithm_info = 0U, .params = internal_params, .has_private_seed = false,
      .has_private_key = false, .private_seed = { .data = { 0U } },
      .private_random = { .data = { 0U } }, .public_seed = { .data = { 0U } },
      .encoded_t = { .data = { 0U } }, .encaps_key_hash = { .data = { 0U } }, .n_rows = n_rows
    };
  core_result_Result_67 uu____0;
  switch (params)
  {
    case symcrust_key_Params_MlKem512:
      {
        symcrust_key_PreKey2_72 lit;
        lit.header = header;
        Eurydice_arr_bc repeat_expression[8U];
        KRML_MAYBE_FOR8(i,
          (size_t)0U,
          (size_t)8U,
          (size_t)1U,
          repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
        memcpy(lit.data.data, repeat_expression, (size_t)8U * sizeof (Eurydice_arr_bc));
        uu____0 =
          (
            KRML_CLITERAL(core_result_Result_67){
              .tag = core_result_Ok,
              .val = {
                .case_Ok = {
                  .ptr = (symcrust_key_PreKey2_46 *)Eurydice_box_new(lit,
                    symcrust_key_PreKey2_72,
                    symcrust_key_PreKey2_72 *),
                  .len = (size_t)8U
                }
              }
            }
          );
        break;
      }
    case symcrust_key_Params_MlKem768:
      {
        symcrust_key_PreKey2_0e lit;
        lit.header = header;
        Eurydice_arr_bc repeat_expression[15U];
        KRML_MAYBE_FOR15(i,
          (size_t)0U,
          (size_t)15U,
          (size_t)1U,
          repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
        memcpy(lit.data.data, repeat_expression, (size_t)15U * sizeof (Eurydice_arr_bc));
        uu____0 =
          (
            KRML_CLITERAL(core_result_Result_67){
              .tag = core_result_Ok,
              .val = {
                .case_Ok = {
                  .ptr = (symcrust_key_PreKey2_46 *)Eurydice_box_new(lit,
                    symcrust_key_PreKey2_0e,
                    symcrust_key_PreKey2_0e *),
                  .len = (size_t)15U
                }
              }
            }
          );
        break;
      }
    case symcrust_key_Params_MlKem1024:
      {
        symcrust_key_PreKey2_b5 lit;
        lit.header = header;
        Eurydice_arr_bc repeat_expression[24U];
        for (size_t i = (size_t)0U; i < (size_t)24U; i++)
        {
          repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;
        }
        memcpy(lit.data.data, repeat_expression, (size_t)24U * sizeof (Eurydice_arr_bc));
        uu____0 =
          (
            KRML_CLITERAL(core_result_Result_67){
              .tag = core_result_Ok,
              .val = {
                .case_Ok = {
                  .ptr = (symcrust_key_PreKey2_46 *)Eurydice_box_new(lit,
                    symcrust_key_PreKey2_b5,
                    symcrust_key_PreKey2_b5 *),
                  .len = (size_t)24U
                }
              }
            }
          );
        break;
      }
    default:
      {
        KRML_HOST_EPRINTF("KaRaMeL incomplete match at %s:%d\n", __FILE__, __LINE__);
        KRML_HOST_EXIT(253U);
      }
  }
  return uu____0;
}

core_result_Result_67 symcrust_key_key_allocate(symcrust_key_Params params)
{
  return symcrust_key_key_allocate2(params);
}

static void
poly_element_sample_ntt_from_shake128(
  symcrust_hash_HashState *p_state,
  Eurydice_arr_bc *pe_dst
)
{
  size_t i = (size_t)0U;
  Eurydice_arr_6d shake_output_buf = { .data = { 0U } };
  size_t
  curr_buf_index =
    Eurydice_slice_len(Eurydice_array_to_slice((size_t)24U, &shake_output_buf, uint8_t),
      uint8_t);
  while (i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS)
  {
    size_t uu____0 = curr_buf_index;
    EURYDICE_ASSERT(uu____0 <=
        Eurydice_slice_len(Eurydice_array_to_slice((size_t)24U, &shake_output_buf, uint8_t),
          uint8_t),
      "panic!");
    size_t uu____1 = curr_buf_index;
    if
    (
      uu____1 ==
        Eurydice_slice_len(Eurydice_array_to_slice((size_t)24U, &shake_output_buf, uint8_t),
          uint8_t)
    )
    {
      /* Note (Rust): shakeOutputBuf[..] seems unnecessary and trips Eurydice (FIXME, see #14) */
      symcrust_hash_shake128_extract(p_state,
        Eurydice_array_to_slice((size_t)24U, &shake_output_buf, uint8_t),
        false);
      curr_buf_index = (size_t)0U;
    }
    uint16_t
    sample0 =
      (uint32_t)core_num__u16__from_le_bytes(symcrust_ntt_slice_to_sub_array((size_t)2U,
          Eurydice_array_to_slice((size_t)24U, &shake_output_buf, uint8_t),
          curr_buf_index,
          Eurydice_arr_8b))
      & 4095U;
    uint16_t
    sample1 =
      (uint32_t)core_num__u16__from_le_bytes(symcrust_ntt_slice_to_sub_array((size_t)2U,
          Eurydice_array_to_slice((size_t)24U, &shake_output_buf, uint8_t),
          curr_buf_index + (size_t)1U,
          Eurydice_arr_8b))
      >> 4U;
    curr_buf_index = curr_buf_index + (size_t)3U;
    pe_dst->data[i] = sample0;
    i = i + (size_t)((uint32_t)sample0 < Q);
    if (i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS)
    {
      /* TODO: Aeneas crashes if we comment the code below this line */
      pe_dst->data[i] = sample1;
      i = i + (size_t)((uint32_t)sample1 < Q);
    }
  }
}

void
symcrust_mlkem_key_expand_public_matrix_from_public_seed(
  Eurydice_dst_6f pk_mlkem_key,
  symcrust_ntt_InternalComputationTemporaries *p_comp_temps
)
{
  Eurydice_arr_8b coordinates = { .data = { 0U } };
  symcrust_hash_HashState *p_shake_state_base = &p_comp_temps->hash_state0;
  symcrust_hash_HashState *p_shake_state_work = &p_comp_temps->hash_state1;
  uint8_t n_rows = pk_mlkem_key.ptr->header.params.n_rows;
  symcrust_hash_shake128_init(p_shake_state_base);
  symcrust_hash_shake128_append(p_shake_state_base,
    Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.public_seed, uint8_t));
  for (uint8_t i = 0U; i < n_rows; i = (uint32_t)i + 1U)
  {
    coordinates.data[1U] = i;
    for (uint8_t j = 0U; j < n_rows; j = (uint32_t)j + 1U)
    {
      coordinates.data[0U] = j;
      symcrust_hash_shake128_state_copy(p_shake_state_base, p_shake_state_work);
      symcrust_hash_shake128_append(p_shake_state_work,
        Eurydice_array_to_slice((size_t)2U, &coordinates, uint8_t));
      Eurydice_slice a_transpose = symcrust_key_atranspose_mut_fd(pk_mlkem_key);
      poly_element_sample_ntt_from_shake128(p_shake_state_work,
        &Eurydice_slice_index(a_transpose,
          (size_t)((uint32_t)i * (uint32_t)n_rows + (uint32_t)j),
          Eurydice_arr_bc,
          Eurydice_arr_bc *));
    }
  }
}

#define RSQR (1353U)

#define RSQR_TIMES_NEG_Q_INV_MOD_R (44983U)

static void poly_element_mul_r(Eurydice_arr_bc *pe_src, Eurydice_arr_bc *pe_dst)
{
  for (size_t i = (size_t)0U; i < SYMCRUST_KEY_MLWE_POLYNOMIAL_COEFFICIENTS; i++)
  {
    uint32_t uu____0 = mont_mul((uint32_t)pe_src->data[i], RSQR, RSQR_TIMES_NEG_Q_INV_MOD_R);
    pe_dst->data[i] = (uint16_t)uu____0;
  }
}

static void vector_mul_r(Eurydice_slice pv_src, Eurydice_slice pv_dst)
{
  size_t n_rows = Eurydice_slice_len(pv_src, Eurydice_arr_bc);
  EURYDICE_ASSERT(n_rows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows <= MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pv_dst, Eurydice_arr_bc) == n_rows, "panic!");
  for (size_t i = (size_t)0U; i < n_rows; i++)
  {
    poly_element_mul_r(&Eurydice_slice_index(pv_src, i, Eurydice_arr_bc, Eurydice_arr_bc *),
      &Eurydice_slice_index(pv_dst, i, Eurydice_arr_bc, Eurydice_arr_bc *));
  }
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x3
symcrust_key_ats_mut_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x2
  uu____0 =
    Eurydice_slice_split_at_mut(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      m_len,
      Eurydice_arr_bc,
      Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x2);
  Eurydice_slice a = uu____0.fst;
  Eurydice_slice ts = uu____0.snd;
  Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x2
  uu____1 =
    Eurydice_slice_split_at_mut(ts,
      self.ptr->header.n_rows,
      Eurydice_arr_bc,
      Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x2);
  Eurydice_slice t = uu____1.fst;
  Eurydice_slice s = uu____1.snd;
  return
    (
      KRML_CLITERAL(Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x3){
        .fst = a,
        .snd = t,
        .thd = s
      }
    );
}

/**
This function found in impl {@Slice<T>}
*/
/**
A monomorphic instance of core.slice.swap_2b
with types Eurydice_arr uint16_t[[$256size_t]]

*/
static void swap_2b_d9(Eurydice_slice s, size_t i, size_t j)
{
  Eurydice_arr_bc tmp = Eurydice_slice_index(s, i, Eurydice_arr_bc, Eurydice_arr_bc *);
  Eurydice_slice_index(s, i, Eurydice_arr_bc, Eurydice_arr_bc *) =
    Eurydice_slice_index(s,
      j,
      Eurydice_arr_bc,
      Eurydice_arr_bc *);
  Eurydice_slice_index(s, j, Eurydice_arr_bc, Eurydice_arr_bc *) = tmp;
}

static void matrix_transpose(Eurydice_slice pm_src, uint8_t n_rows)
{
  size_t n_rows0 = (size_t)n_rows;
  EURYDICE_ASSERT(n_rows0 > (size_t)0U, "panic!");
  EURYDICE_ASSERT(n_rows0 <= MATRIX_MAX_NROWS, "panic!");
  for (size_t i = (size_t)0U; i < n_rows0; i++)
  {
    for (size_t j = i + (size_t)1U; j < n_rows0; j++)
    {
      swap_2b_d9(pm_src, i * n_rows0 + j, j * n_rows0 + i);
    }
  }
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
tuple_35 symcrust_key_t_encoded_t_mut_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    (
      KRML_CLITERAL(tuple_35){
        .fst = Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
            self.len,
            Eurydice_arr_bc,
            Eurydice_slice),
          m_len,
          m_len + self.ptr->header.n_rows,
          Eurydice_arr_bc *),
        .snd = &self.ptr->header.encoded_t
      }
    );
}

void
symcrust_mlkem_key_compute_encapsulation_key_hash(
  Eurydice_dst_6f pk_mlkem_key,
  symcrust_ntt_InternalComputationTemporaries *p_comp_temps
)
{
  symcrust_hash_HashState *p_state = &p_comp_temps->hash_state0;
  size_t
  cb_encoded_vector =
    symcrust_mlkem_sizeof_encoded_uncompressed_vector((size_t)pk_mlkem_key.ptr->header.params.n_rows);
  symcrust_hash_sha3_256_init(p_state);
  symcrust_hash_sha3_256_append(p_state,
    Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
      (size_t)0U,
      cb_encoded_vector,
      uint8_t *));
  symcrust_hash_sha3_256_append(p_state,
    Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.public_seed, uint8_t));
  symcrust_hash_sha3_256_result(p_state, &pk_mlkem_key.ptr->header.encaps_key_hash);
}

/**
A monomorphic instance of symcrust.common.wipe_slice
with types uint8_t

*/
void symcrust_common_wipe_slice_90(Eurydice_slice pb_dst)
{
  uint8_t *uu____0 = (uint8_t *)core_slice___Slice_T___as_mut_ptr(pb_dst, uint8_t, uint8_t *);
  size_t uu____1 = Eurydice_slice_len(pb_dst, uint8_t);
  symcrust_common_wipe(uu____0, uu____1 * core_mem_size_of(uint8_t, size_t));
}

void
symcrust_mlkem_key_expand_from_private_seed(
  Eurydice_dst_6f pk_mlkem_key,
  symcrust_ntt_InternalComputationTemporaries *p_comp_temps
)
{
  Eurydice_arr_06 private_seed_hash = { .data = { 0U } };
  Eurydice_arr_73 cbd_sample_buffer = { .data = { 0U } };
  /* PVECTOR pv_tmp;
    PPOLYELEMENT_ACCUMULATOR pa_tmp;
    UINT32 i; */
  uint8_t n_rows = pk_mlkem_key.ptr->header.params.n_rows;
  uint8_t n_eta1 = pk_mlkem_key.ptr->header.params.n_eta1;
  size_t cb_encoded_vector = symcrust_mlkem_sizeof_encoded_uncompressed_vector((size_t)n_rows);
  EURYDICE_ASSERT(!!pk_mlkem_key.ptr->header.has_private_seed, "assert failure");
  if (!(n_eta1 == 2U))
  {
    EURYDICE_ASSERT(n_eta1 == 3U, "panic!");
  }
  Eurydice_arr_73 *uu____0 = &cbd_sample_buffer;
  size_t
  uu____1 =
    Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
        &pk_mlkem_key.ptr->header.private_seed,
        uint8_t),
      uint8_t);
  Eurydice_slice_copy(Eurydice_array_to_subslice3(uu____0, (size_t)0U, uu____1, uint8_t *),
    Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.private_seed, uint8_t),
    uint8_t);
  uint8_t uu____2 = n_rows;
  size_t
  uu____3 =
    Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
        &pk_mlkem_key.ptr->header.private_seed,
        uint8_t),
      uint8_t);
  cbd_sample_buffer.data[uu____3] = uu____2;
  Eurydice_arr_73 *uu____4 = &cbd_sample_buffer;
  symcrust_hash_sha3_512(Eurydice_array_to_subslice3(uu____4,
      (size_t)0U,
      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
          &pk_mlkem_key.ptr->header.private_seed,
          uint8_t),
        uint8_t)
      + (size_t)1U,
      uint8_t *),
    &private_seed_hash);
  size_t
  pk_len =
    Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
        &pk_mlkem_key.ptr->header.public_seed,
        uint8_t),
      uint8_t);
  Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
      &pk_mlkem_key.ptr->header.public_seed,
      uint8_t),
    Eurydice_array_to_subslice3(&private_seed_hash, (size_t)0U, pk_len, uint8_t *),
    uint8_t);
  symcrust_mlkem_key_expand_public_matrix_from_public_seed(pk_mlkem_key, p_comp_temps);
  symcrust_hash_shake256_init(&p_comp_temps->hash_state0);
  symcrust_hash_HashState *uu____5 = &p_comp_temps->hash_state0;
  Eurydice_arr_06 *uu____6 = &private_seed_hash;
  size_t
  uu____7 =
    Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
        &pk_mlkem_key.ptr->header.public_seed,
        uint8_t),
      uint8_t);
  symcrust_hash_shake256_append(uu____5,
    Eurydice_array_to_subslice3(uu____6,
      uu____7,
      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
          &pk_mlkem_key.ptr->header.public_seed,
          uint8_t),
        uint8_t)
      + (size_t)32U,
      uint8_t *));
  for (uint8_t i = 0U; i < n_rows; i = (uint32_t)i + 1U)
  {
    cbd_sample_buffer.data[0U] = i;
    symcrust_hash_shake256_state_copy(&p_comp_temps->hash_state0, &p_comp_temps->hash_state1);
    symcrust_hash_shake256_append(&p_comp_temps->hash_state1,
      Eurydice_array_to_subslice3(&cbd_sample_buffer, (size_t)0U, (size_t)1U, uint8_t *));
    symcrust_hash_shake256_extract(&p_comp_temps->hash_state1,
      Eurydice_array_to_subslice3(&cbd_sample_buffer,
        (size_t)0U,
        (size_t)64U * (size_t)n_eta1,
        uint8_t *),
      false);
    poly_element_sample_cbd_from_bytes(Eurydice_array_to_slice((size_t)193U,
        &cbd_sample_buffer,
        uint8_t),
      (uint32_t)n_eta1,
      &Eurydice_slice_index(symcrust_key_s_mut_fd(pk_mlkem_key),
        (size_t)i,
        Eurydice_arr_bc,
        Eurydice_arr_bc *));
  }
  for (uint8_t i = 0U; i < n_rows; i = (uint32_t)i + 1U)
  {
    cbd_sample_buffer.data[0U] = (uint32_t)n_rows + (uint32_t)i;
    symcrust_hash_shake256_state_copy(&p_comp_temps->hash_state0, &p_comp_temps->hash_state1);
    symcrust_hash_shake256_append(&p_comp_temps->hash_state1,
      Eurydice_array_to_subslice3(&cbd_sample_buffer, (size_t)0U, (size_t)1U, uint8_t *));
    symcrust_hash_shake256_extract(&p_comp_temps->hash_state1,
      Eurydice_array_to_subslice3(&cbd_sample_buffer,
        (size_t)0U,
        (size_t)64U * (size_t)n_eta1,
        uint8_t *),
      false);
    poly_element_sample_cbd_from_bytes(Eurydice_array_to_slice((size_t)193U,
        &cbd_sample_buffer,
        uint8_t),
      (uint32_t)n_eta1,
      &Eurydice_slice_index(symcrust_key_t_mut_fd(pk_mlkem_key),
        (size_t)i,
        Eurydice_arr_bc,
        Eurydice_arr_bc *));
  }
  vector_ntt(symcrust_key_s_mut_fd(pk_mlkem_key));
  vector_ntt(symcrust_key_t_mut_fd(pk_mlkem_key));
  Eurydice_slice
  pv_tmp =
    Eurydice_array_to_subslice3(&p_comp_temps->ab_vector_buffer0,
      (size_t)0U,
      (size_t)n_rows,
      Eurydice_arr_bc *);
  vector_mul_r(symcrust_key_s_mut_fd(pk_mlkem_key), pv_tmp);
  Eurydice_slice_Eurydice_arr_uint16_t___256size_t___x3
  uu____8 = symcrust_key_ats_mut_fd(pk_mlkem_key);
  Eurydice_slice a = uu____8.fst;
  Eurydice_slice t0 = uu____8.snd;
  Eurydice_arr_82 *pa_tmp = &p_comp_temps->ab_poly_element_accumulator_buffer;
  matrix_vector_mont_mul_and_add(a,
    Eurydice_array_to_subslice3(&p_comp_temps->ab_vector_buffer0,
      (size_t)0U,
      (size_t)n_rows,
      Eurydice_arr_bc *),
    t0,
    pa_tmp,
    n_rows);
  matrix_transpose(symcrust_key_atranspose_mut_fd(pk_mlkem_key), n_rows);
  tuple_35 uu____9 = symcrust_key_t_encoded_t_mut_fd(pk_mlkem_key);
  Eurydice_slice t = uu____9.fst;
  Eurydice_arr_38 *encoded_t = uu____9.snd;
  vector_compress_and_encode(t,
    12U,
    Eurydice_array_to_subslice3(encoded_t, (size_t)0U, cb_encoded_vector, uint8_t *));
  symcrust_mlkem_key_compute_encapsulation_key_hash(pk_mlkem_key, p_comp_temps);
  symcrust_common_wipe_slice_90(Eurydice_array_to_slice((size_t)64U, &private_seed_hash, uint8_t));
  symcrust_common_wipe_slice_90(Eurydice_array_to_slice((size_t)193U,
      &cbd_sample_buffer,
      uint8_t));
}

symcrust_common_Error
symcrust_mlkem_key_set_value(
  Eurydice_slice pb_src,
  symcrust_key_Format format,
  uint32_t flags,
  Eurydice_dst_6f pk_mlkem_key
)
{
  /* ERROR sc_error = NO_ERROR; */
  size_t pb_curr = (size_t)0U;
  /* PINTERNAL_COMPUTATION_TEMPORARIES p_comp_temps = NULL; */
  uint8_t n_rows = pk_mlkem_key.ptr->header.params.n_rows;
  size_t cb_encoded_vector = symcrust_mlkem_sizeof_encoded_uncompressed_vector((size_t)n_rows);
  /* Ensure only allowed flags are specified */
  uint32_t
  allowed_flags = SYMCRUST_MLKEM_FLAG_KEY_NO_FIPS | SYMCRUST_MLKEM_FLAG_KEY_MINIMAL_VALIDATION;
  symcrust_common_Error uu____0;
  if ((flags & ~allowed_flags) != 0U)
  {
    uu____0 = symcrust_common_Error_InvalidArgument;
  }
  else
  {
    /* Check that minimal validation flag only specified with no fips */
    if ((flags & SYMCRUST_MLKEM_FLAG_KEY_NO_FIPS) == 0U)
    {
      if ((flags & SYMCRUST_MLKEM_FLAG_KEY_MINIMAL_VALIDATION) != 0U)
      {
        uu____0 = symcrust_common_Error_InvalidArgument;
      }
    }
    /* Note (Rust): ruled out by typing
      if( mlKemkeyFormat == crate::key::Format_NULL )
      {
          return MLKEM_ERROR::INVALID_ARGUMENT;
      } */
    if ((flags & SYMCRUST_MLKEM_FLAG_KEY_NO_FIPS) == 0U)
    {
      symcrust_ntt_InternalComputationTemporaries lit;
      Eurydice_arr_bc repeat_expression[4U];
      KRML_MAYBE_FOR4(i,
        (size_t)0U,
        (size_t)4U,
        (size_t)1U,
        repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
      memcpy(lit.ab_vector_buffer0.data, repeat_expression, (size_t)4U * sizeof (Eurydice_arr_bc));
      Eurydice_arr_bc repeat_expression0[4U];
      KRML_MAYBE_FOR4(i,
        (size_t)0U,
        (size_t)4U,
        (size_t)1U,
        repeat_expression0[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
      memcpy(lit.ab_vector_buffer1.data, repeat_expression0, (size_t)4U * sizeof (Eurydice_arr_bc));
      lit.ab_poly_element_buffer0 = SYMCRUST_KEY_POLYELEMENT_ZERO;
      lit.ab_poly_element_buffer1 = SYMCRUST_KEY_POLYELEMENT_ZERO;
      memset(lit.ab_poly_element_accumulator_buffer.data, 0U, 256U * sizeof (uint32_t));
      lit.hash_state0 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
      lit.hash_state1 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
      core_result_Result_80
      p_comp_temps =
        alloc_boxed__alloc__boxed__Box_T___try_new(lit,
          symcrust_ntt_InternalComputationTemporaries,
          core_result_Result_80);
      if (p_comp_temps.tag == core_result_Ok)
      {
        symcrust_ntt_InternalComputationTemporaries *p_comp_temps0 = p_comp_temps.f0;
        symcrust_ntt_InternalComputationTemporaries *p_comp_temps1 = p_comp_temps0;
        bool uu____1;
        bool uu____2;
        size_t uu____3;
        size_t uu____4;
        Eurydice_slice uu____5;
        switch (format)
        {
          case symcrust_key_Format_PrivateSeed:
            {
              if
              (!(Eurydice_slice_len(pb_src, uint8_t) != SYMCRUST_MLKEM_SIZEOF_FORMAT_PRIVATE_SEED))
              {
                pk_mlkem_key.ptr->header.has_private_seed = true;
                size_t
                l =
                  Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                      &pk_mlkem_key.ptr->header.private_seed,
                      uint8_t),
                    uint8_t);
                Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                    &pk_mlkem_key.ptr->header.private_seed,
                    uint8_t),
                  Eurydice_slice_subslice3(pb_src, (size_t)0U, l, uint8_t *),
                  uint8_t);
                pb_curr = pb_curr + l;
                pk_mlkem_key.ptr->header.has_private_key = true;
                size_t
                l0 =
                  Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                      &pk_mlkem_key.ptr->header.private_random,
                      uint8_t),
                    uint8_t);
                Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                    &pk_mlkem_key.ptr->header.private_random,
                    uint8_t),
                  Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l0, uint8_t *),
                  uint8_t);
                pb_curr = pb_curr + l0;
                symcrust_mlkem_key_expand_from_private_seed(pk_mlkem_key, p_comp_temps1);
                uu____1 = true;
                if (uu____1)
                {
                  /* Note (Rust): exhaustiveness
                    else
                    {
                        sc_error = NOT_IMPLEMENTED;
                        goto cleanup;
                    } */
                  uu____3 = pb_curr;
                  uu____5 = pb_src;
                  uu____4 = Eurydice_slice_len(uu____5, uint8_t);
                  uu____2 = uu____3 == uu____4;
                  EURYDICE_ASSERT(uu____2, "panic!");
                }
                return symcrust_common_Error_NoError;
              }
              uu____0 = symcrust_common_Error_WrongKeySize;
              break;
            }
          case symcrust_key_Format_DecapsulationKey:
            {
              size_t uu____6 = Eurydice_slice_len(pb_src, uint8_t);
              if (uu____6 != symcrust_mlkem_sizeof_format_decapsulation_key((size_t)n_rows))
              {
                uu____0 = symcrust_common_Error_WrongKeySize;
              }
              else
              {
                symcrust_common_Error
                sc_error =
                  vector_decode_and_decompress(Eurydice_slice_subslice3(pb_src,
                      pb_curr,
                      pb_curr + cb_encoded_vector,
                      uint8_t *),
                    12U,
                    symcrust_key_s_mut_fd(pk_mlkem_key));
                /* original Rust expression is not an lvalue in C */
                symcrust_common_Error lvalue0 = symcrust_common_Error_NoError;
                if
                (
                  symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error,
                    &lvalue0)
                )
                {
                  uu____0 = sc_error;
                }
                else
                {
                  pb_curr = pb_curr + cb_encoded_vector;
                  Eurydice_slice_copy(Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
                      (size_t)0U,
                      cb_encoded_vector,
                      uint8_t *),
                    Eurydice_slice_subslice3(pb_src,
                      pb_curr,
                      pb_curr + cb_encoded_vector,
                      uint8_t *),
                    uint8_t);
                  pb_curr = pb_curr + cb_encoded_vector;
                  tuple_35 uu____7 = symcrust_key_t_encoded_t_mut_fd(pk_mlkem_key);
                  Eurydice_slice t = uu____7.fst;
                  Eurydice_arr_38 *encoded_t = uu____7.snd;
                  symcrust_common_Error
                  sc_error0 =
                    vector_decode_and_decompress(Eurydice_array_to_subslice3(encoded_t,
                        (size_t)0U,
                        cb_encoded_vector,
                        uint8_t *),
                      12U,
                      t);
                  /* original Rust expression is not an lvalue in C */
                  symcrust_common_Error lvalue = symcrust_common_Error_NoError;
                  if
                  (
                    !symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error0,
                      &lvalue)
                  )
                  {
                    size_t
                    l =
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.public_seed,
                          uint8_t),
                        uint8_t);
                    Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.public_seed,
                        uint8_t),
                      Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l, uint8_t *),
                      uint8_t);
                    pb_curr =
                      pb_curr +
                        Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                            &pk_mlkem_key.ptr->header.public_seed,
                            uint8_t),
                          uint8_t);
                    symcrust_mlkem_key_expand_public_matrix_from_public_seed(pk_mlkem_key,
                      p_comp_temps1);
                    matrix_transpose(symcrust_key_atranspose_mut_fd(pk_mlkem_key), n_rows);
                    size_t
                    l0 =
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.encaps_key_hash,
                          uint8_t),
                        uint8_t);
                    Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.encaps_key_hash,
                        uint8_t),
                      Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l0, uint8_t *),
                      uint8_t);
                    pb_curr =
                      pb_curr +
                        Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                            &pk_mlkem_key.ptr->header.encaps_key_hash,
                            uint8_t),
                          uint8_t);
                    size_t
                    l1 =
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.private_random,
                          uint8_t),
                        uint8_t);
                    Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.private_random,
                        uint8_t),
                      Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l1, uint8_t *),
                      uint8_t);
                    pb_curr =
                      pb_curr +
                        Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                            &pk_mlkem_key.ptr->header.private_random,
                            uint8_t),
                          uint8_t);
                    pk_mlkem_key.ptr->header.has_private_seed = false;
                    pk_mlkem_key.ptr->header.has_private_key = true;
                    uu____1 = true;
                    if (uu____1)
                    {
                      uu____3 = pb_curr;
                      uu____5 = pb_src;
                      uu____4 = Eurydice_slice_len(uu____5, uint8_t);
                      uu____2 = uu____3 == uu____4;
                      EURYDICE_ASSERT(uu____2, "panic!");
                    }
                    return symcrust_common_Error_NoError;
                  }
                  uu____0 = sc_error0;
                }
              }
              break;
            }
          case symcrust_key_Format_EncapsulationKey:
            {
              size_t uu____8 = Eurydice_slice_len(pb_src, uint8_t);
              if (uu____8 != symcrust_mlkem_sizeof_format_encapsulation_key((size_t)n_rows))
              {
                uu____0 = symcrust_common_Error_WrongKeySize;
              }
              else
              {
                Eurydice_slice_copy(Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
                    (size_t)0U,
                    cb_encoded_vector,
                    uint8_t *),
                  Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + cb_encoded_vector, uint8_t *),
                  uint8_t);
                pb_curr = pb_curr + cb_encoded_vector;
                tuple_35 uu____9 = symcrust_key_t_encoded_t_mut_fd(pk_mlkem_key);
                Eurydice_slice t = uu____9.fst;
                Eurydice_arr_38 *encoded_t = uu____9.snd;
                symcrust_common_Error
                sc_error =
                  vector_decode_and_decompress(Eurydice_array_to_subslice3(encoded_t,
                      (size_t)0U,
                      cb_encoded_vector,
                      uint8_t *),
                    12U,
                    t);
                /* original Rust expression is not an lvalue in C */
                symcrust_common_Error lvalue = symcrust_common_Error_NoError;
                if
                (
                  !symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error,
                    &lvalue)
                )
                {
                  size_t
                  l =
                    Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.public_seed,
                        uint8_t),
                      uint8_t);
                  Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                      &pk_mlkem_key.ptr->header.public_seed,
                      uint8_t),
                    Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l, uint8_t *),
                    uint8_t);
                  pb_curr =
                    pb_curr +
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.public_seed,
                          uint8_t),
                        uint8_t);
                  symcrust_mlkem_key_expand_public_matrix_from_public_seed(pk_mlkem_key,
                    p_comp_temps1);
                  matrix_transpose(symcrust_key_atranspose_mut_fd(pk_mlkem_key), n_rows);
                  symcrust_mlkem_key_compute_encapsulation_key_hash(pk_mlkem_key, p_comp_temps1);
                  pk_mlkem_key.ptr->header.has_private_seed = false;
                  pk_mlkem_key.ptr->header.has_private_key = false;
                  uu____1 = true;
                  if (uu____1)
                  {
                    uu____3 = pb_curr;
                    uu____5 = pb_src;
                    uu____4 = Eurydice_slice_len(uu____5, uint8_t);
                    uu____2 = uu____3 == uu____4;
                    EURYDICE_ASSERT(uu____2, "panic!");
                  }
                  return symcrust_common_Error_NoError;
                }
                uu____0 = sc_error;
              }
              break;
            }
          default:
            {
              KRML_HOST_EPRINTF("KaRaMeL incomplete match at %s:%d\n", __FILE__, __LINE__);
              KRML_HOST_EXIT(253U);
            }
        }
      }
      else
      {
        uu____0 = symcrust_common_Error_MemoryAllocationFailure;
      }
    }
    else
    {
      symcrust_ntt_InternalComputationTemporaries lit;
      Eurydice_arr_bc repeat_expression[4U];
      KRML_MAYBE_FOR4(i,
        (size_t)0U,
        (size_t)4U,
        (size_t)1U,
        repeat_expression[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
      memcpy(lit.ab_vector_buffer0.data, repeat_expression, (size_t)4U * sizeof (Eurydice_arr_bc));
      Eurydice_arr_bc repeat_expression0[4U];
      KRML_MAYBE_FOR4(i,
        (size_t)0U,
        (size_t)4U,
        (size_t)1U,
        repeat_expression0[i] = SYMCRUST_KEY_POLYELEMENT_ZERO;);
      memcpy(lit.ab_vector_buffer1.data, repeat_expression0, (size_t)4U * sizeof (Eurydice_arr_bc));
      lit.ab_poly_element_buffer0 = SYMCRUST_KEY_POLYELEMENT_ZERO;
      lit.ab_poly_element_buffer1 = SYMCRUST_KEY_POLYELEMENT_ZERO;
      memset(lit.ab_poly_element_accumulator_buffer.data, 0U, 256U * sizeof (uint32_t));
      lit.hash_state0 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
      lit.hash_state1 = SYMCRUST_HASH_UNINITIALIZED_HASH_STATE;
      core_result_Result_80
      p_comp_temps =
        alloc_boxed__alloc__boxed__Box_T___try_new(lit,
          symcrust_ntt_InternalComputationTemporaries,
          core_result_Result_80);
      if (p_comp_temps.tag == core_result_Ok)
      {
        symcrust_ntt_InternalComputationTemporaries *p_comp_temps0 = p_comp_temps.f0;
        symcrust_ntt_InternalComputationTemporaries *p_comp_temps1 = p_comp_temps0;
        bool uu____10;
        bool uu____11;
        size_t uu____12;
        size_t uu____13;
        Eurydice_slice uu____14;
        switch (format)
        {
          case symcrust_key_Format_PrivateSeed:
            {
              if
              (!(Eurydice_slice_len(pb_src, uint8_t) != SYMCRUST_MLKEM_SIZEOF_FORMAT_PRIVATE_SEED))
              {
                pk_mlkem_key.ptr->header.has_private_seed = true;
                size_t
                l =
                  Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                      &pk_mlkem_key.ptr->header.private_seed,
                      uint8_t),
                    uint8_t);
                Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                    &pk_mlkem_key.ptr->header.private_seed,
                    uint8_t),
                  Eurydice_slice_subslice3(pb_src, (size_t)0U, l, uint8_t *),
                  uint8_t);
                pb_curr = pb_curr + l;
                pk_mlkem_key.ptr->header.has_private_key = true;
                size_t
                l0 =
                  Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                      &pk_mlkem_key.ptr->header.private_random,
                      uint8_t),
                    uint8_t);
                Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                    &pk_mlkem_key.ptr->header.private_random,
                    uint8_t),
                  Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l0, uint8_t *),
                  uint8_t);
                pb_curr = pb_curr + l0;
                symcrust_mlkem_key_expand_from_private_seed(pk_mlkem_key, p_comp_temps1);
                uu____10 = true;
                if (uu____10)
                {
                  uu____12 = pb_curr;
                  uu____14 = pb_src;
                  uu____13 = Eurydice_slice_len(uu____14, uint8_t);
                  uu____11 = uu____12 == uu____13;
                  EURYDICE_ASSERT(uu____11, "panic!");
                }
                return symcrust_common_Error_NoError;
              }
              uu____0 = symcrust_common_Error_WrongKeySize;
              break;
            }
          case symcrust_key_Format_DecapsulationKey:
            {
              size_t uu____15 = Eurydice_slice_len(pb_src, uint8_t);
              if (uu____15 != symcrust_mlkem_sizeof_format_decapsulation_key((size_t)n_rows))
              {
                uu____0 = symcrust_common_Error_WrongKeySize;
              }
              else
              {
                symcrust_common_Error
                sc_error =
                  vector_decode_and_decompress(Eurydice_slice_subslice3(pb_src,
                      pb_curr,
                      pb_curr + cb_encoded_vector,
                      uint8_t *),
                    12U,
                    symcrust_key_s_mut_fd(pk_mlkem_key));
                /* original Rust expression is not an lvalue in C */
                symcrust_common_Error lvalue0 = symcrust_common_Error_NoError;
                if
                (
                  symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error,
                    &lvalue0)
                )
                {
                  uu____0 = sc_error;
                }
                else
                {
                  pb_curr = pb_curr + cb_encoded_vector;
                  Eurydice_slice_copy(Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
                      (size_t)0U,
                      cb_encoded_vector,
                      uint8_t *),
                    Eurydice_slice_subslice3(pb_src,
                      pb_curr,
                      pb_curr + cb_encoded_vector,
                      uint8_t *),
                    uint8_t);
                  pb_curr = pb_curr + cb_encoded_vector;
                  tuple_35 uu____16 = symcrust_key_t_encoded_t_mut_fd(pk_mlkem_key);
                  Eurydice_slice t = uu____16.fst;
                  Eurydice_arr_38 *encoded_t = uu____16.snd;
                  symcrust_common_Error
                  sc_error0 =
                    vector_decode_and_decompress(Eurydice_array_to_subslice3(encoded_t,
                        (size_t)0U,
                        cb_encoded_vector,
                        uint8_t *),
                      12U,
                      t);
                  /* original Rust expression is not an lvalue in C */
                  symcrust_common_Error lvalue = symcrust_common_Error_NoError;
                  if
                  (
                    !symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error0,
                      &lvalue)
                  )
                  {
                    size_t
                    l =
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.public_seed,
                          uint8_t),
                        uint8_t);
                    Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.public_seed,
                        uint8_t),
                      Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l, uint8_t *),
                      uint8_t);
                    pb_curr =
                      pb_curr +
                        Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                            &pk_mlkem_key.ptr->header.public_seed,
                            uint8_t),
                          uint8_t);
                    symcrust_mlkem_key_expand_public_matrix_from_public_seed(pk_mlkem_key,
                      p_comp_temps1);
                    matrix_transpose(symcrust_key_atranspose_mut_fd(pk_mlkem_key), n_rows);
                    size_t
                    l0 =
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.encaps_key_hash,
                          uint8_t),
                        uint8_t);
                    Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.encaps_key_hash,
                        uint8_t),
                      Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l0, uint8_t *),
                      uint8_t);
                    pb_curr =
                      pb_curr +
                        Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                            &pk_mlkem_key.ptr->header.encaps_key_hash,
                            uint8_t),
                          uint8_t);
                    size_t
                    l1 =
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.private_random,
                          uint8_t),
                        uint8_t);
                    Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.private_random,
                        uint8_t),
                      Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l1, uint8_t *),
                      uint8_t);
                    pb_curr =
                      pb_curr +
                        Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                            &pk_mlkem_key.ptr->header.private_random,
                            uint8_t),
                          uint8_t);
                    pk_mlkem_key.ptr->header.has_private_seed = false;
                    pk_mlkem_key.ptr->header.has_private_key = true;
                    uu____10 = true;
                    if (uu____10)
                    {
                      uu____12 = pb_curr;
                      uu____14 = pb_src;
                      uu____13 = Eurydice_slice_len(uu____14, uint8_t);
                      uu____11 = uu____12 == uu____13;
                      EURYDICE_ASSERT(uu____11, "panic!");
                    }
                    return symcrust_common_Error_NoError;
                  }
                  uu____0 = sc_error0;
                }
              }
              break;
            }
          case symcrust_key_Format_EncapsulationKey:
            {
              size_t uu____17 = Eurydice_slice_len(pb_src, uint8_t);
              if (uu____17 != symcrust_mlkem_sizeof_format_encapsulation_key((size_t)n_rows))
              {
                uu____0 = symcrust_common_Error_WrongKeySize;
              }
              else
              {
                Eurydice_slice_copy(Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
                    (size_t)0U,
                    cb_encoded_vector,
                    uint8_t *),
                  Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + cb_encoded_vector, uint8_t *),
                  uint8_t);
                pb_curr = pb_curr + cb_encoded_vector;
                tuple_35 uu____18 = symcrust_key_t_encoded_t_mut_fd(pk_mlkem_key);
                Eurydice_slice t = uu____18.fst;
                Eurydice_arr_38 *encoded_t = uu____18.snd;
                symcrust_common_Error
                sc_error =
                  vector_decode_and_decompress(Eurydice_array_to_subslice3(encoded_t,
                      (size_t)0U,
                      cb_encoded_vector,
                      uint8_t *),
                    12U,
                    t);
                /* original Rust expression is not an lvalue in C */
                symcrust_common_Error lvalue = symcrust_common_Error_NoError;
                if
                (
                  !symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error,
                    &lvalue)
                )
                {
                  size_t
                  l =
                    Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                        &pk_mlkem_key.ptr->header.public_seed,
                        uint8_t),
                      uint8_t);
                  Eurydice_slice_copy(Eurydice_array_to_slice((size_t)32U,
                      &pk_mlkem_key.ptr->header.public_seed,
                      uint8_t),
                    Eurydice_slice_subslice3(pb_src, pb_curr, pb_curr + l, uint8_t *),
                    uint8_t);
                  pb_curr =
                    pb_curr +
                      Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                          &pk_mlkem_key.ptr->header.public_seed,
                          uint8_t),
                        uint8_t);
                  symcrust_mlkem_key_expand_public_matrix_from_public_seed(pk_mlkem_key,
                    p_comp_temps1);
                  matrix_transpose(symcrust_key_atranspose_mut_fd(pk_mlkem_key), n_rows);
                  symcrust_mlkem_key_compute_encapsulation_key_hash(pk_mlkem_key, p_comp_temps1);
                  pk_mlkem_key.ptr->header.has_private_seed = false;
                  pk_mlkem_key.ptr->header.has_private_key = false;
                  uu____10 = true;
                  if (uu____10)
                  {
                    uu____12 = pb_curr;
                    uu____14 = pb_src;
                    uu____13 = Eurydice_slice_len(uu____14, uint8_t);
                    uu____11 = uu____12 == uu____13;
                    EURYDICE_ASSERT(uu____11, "panic!");
                  }
                  return symcrust_common_Error_NoError;
                }
                uu____0 = sc_error;
              }
              break;
            }
          default:
            {
              KRML_HOST_EPRINTF("KaRaMeL incomplete match at %s:%d\n", __FILE__, __LINE__);
              KRML_HOST_EXIT(253U);
            }
        }
      }
      else
      {
        uu____0 = symcrust_common_Error_MemoryAllocationFailure;
      }
    }
  }
  return uu____0;
}

symcrust_common_Error symcrust_mlkem_key_generate(Eurydice_dst_6f pk_mlkem_key, uint32_t flags)
{
  /* ERROR sc_error = NO_ERROR; */
  Eurydice_arr_06 private_seed = { .data = { 0U } };
  /* Ensure only allowed flags are specified */
  uint32_t allowed_flags = SYMCRUST_MLKEM_FLAG_KEY_NO_FIPS;
  symcrust_common_Error uu____0;
  if ((flags & ~allowed_flags) != 0U)
  {
    uu____0 = symcrust_common_Error_InvalidArgument;
  }
  else
  {
    symcrust_common_Error
    sc_error = symcrust_common_random(Eurydice_array_to_slice((size_t)64U, &private_seed, uint8_t));
    /* original Rust expression is not an lvalue in C */
    symcrust_common_Error lvalue0 = symcrust_common_Error_NoError;
    if
    (
      symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error,
        &lvalue0)
    )
    {
      uu____0 = sc_error;
    }
    else
    {
      symcrust_common_Error
      sc_error0 =
        symcrust_mlkem_key_set_value(Eurydice_array_to_slice((size_t)64U, &private_seed, uint8_t),
          symcrust_key_Format_PrivateSeed,
          flags,
          pk_mlkem_key);
      /* original Rust expression is not an lvalue in C */
      symcrust_common_Error lvalue = symcrust_common_Error_NoError;
      if
      (
        !symcrust_common__core__cmp__PartialEq_symcrust__common__Error__for_symcrust__common__Error__ne(&sc_error0,
          &lvalue)
      )
      {
        return symcrust_common_Error_NoError;
      }
      uu____0 = sc_error0;
    }
  }
  return uu____0;
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice symcrust_key_s_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      m_len + self.ptr->header.n_rows,
      m_len + (size_t)2U * self.ptr->header.n_rows,
      Eurydice_arr_bc *);
}

symcrust_common_Error
symcrust_mlkem_key_get_value(
  Eurydice_dst_6f pk_mlkem_key,
  Eurydice_slice pb_dst,
  symcrust_key_Format format,
  uint32_t _flags
)
{
  /* SIZE_T                      cb_dst,
    ERROR sc_error = NO_ERROR; */
  size_t pb_curr = (size_t)0U;
  uint8_t n_rows = pk_mlkem_key.ptr->header.params.n_rows;
  size_t cb_encoded_vector = symcrust_mlkem_sizeof_encoded_uncompressed_vector((size_t)n_rows);
  symcrust_common_Error uu____0;
  bool uu____1;
  bool uu____2;
  size_t uu____3;
  size_t uu____4;
  Eurydice_slice uu____5;
  switch (format)
  {
    case symcrust_key_Format_PrivateSeed:
      {
        if (Eurydice_slice_len(pb_dst, uint8_t) != SYMCRUST_MLKEM_SIZEOF_FORMAT_PRIVATE_SEED)
        {
          uu____0 = symcrust_common_Error_WrongKeySize;
        }
        else if (pk_mlkem_key.ptr->header.has_private_seed)
        {
          Eurydice_slice uu____6 = pb_dst;
          size_t uu____7 = pb_curr;
          size_t uu____8 = pb_curr;
          size_t
          uu____9 =
            Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.private_seed,
                uint8_t),
              uint8_t);
          Eurydice_slice_copy(Eurydice_slice_subslice3(uu____6,
              uu____7,
              uu____8 + uu____9,
              uint8_t *),
            Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.private_seed, uint8_t),
            uint8_t);
          pb_curr =
            pb_curr +
              Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                  &pk_mlkem_key.ptr->header.private_seed,
                  uint8_t),
                uint8_t);
          Eurydice_slice uu____10 = pb_dst;
          size_t uu____11 = pb_curr;
          size_t uu____12 = pb_curr;
          size_t
          uu____13 =
            Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.private_random,
                uint8_t),
              uint8_t);
          Eurydice_slice_copy(Eurydice_slice_subslice3(uu____10,
              uu____11,
              uu____12 + uu____13,
              uint8_t *),
            Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.private_random, uint8_t),
            uint8_t);
          pb_curr =
            pb_curr +
              Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                  &pk_mlkem_key.ptr->header.private_random,
                  uint8_t),
                uint8_t);
          uu____1 = true;
          if (uu____1)
          {
            uu____3 = pb_curr;
            uu____5 = pb_dst;
            uu____4 = Eurydice_slice_len(uu____5, uint8_t);
            uu____2 = uu____3 == uu____4;
            EURYDICE_ASSERT(uu____2, "panic!");
          }
          return symcrust_common_Error_NoError;
        }
        else
        {
          uu____0 = symcrust_common_Error_IncompatibleFormat;
        }
        break;
      }
    case symcrust_key_Format_DecapsulationKey:
      {
        size_t uu____14 = Eurydice_slice_len(pb_dst, uint8_t);
        if (uu____14 != symcrust_mlkem_sizeof_format_decapsulation_key((size_t)n_rows))
        {
          uu____0 = symcrust_common_Error_InvalidArgument;
        }
        else if (pk_mlkem_key.ptr->header.has_private_key)
        {
          vector_compress_and_encode(symcrust_key_s_fd(pk_mlkem_key),
            12U,
            Eurydice_slice_subslice3(pb_dst, (size_t)0U, cb_encoded_vector, uint8_t *));
          pb_curr = pb_curr + cb_encoded_vector;
          Eurydice_slice_copy(Eurydice_slice_subslice3(pb_dst,
              pb_curr,
              pb_curr + cb_encoded_vector,
              uint8_t *),
            Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
              (size_t)0U,
              cb_encoded_vector,
              uint8_t *),
            uint8_t);
          pb_curr = pb_curr + cb_encoded_vector;
          Eurydice_slice uu____15 = pb_dst;
          size_t uu____16 = pb_curr;
          size_t uu____17 = pb_curr;
          size_t
          uu____18 =
            Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.public_seed,
                uint8_t),
              uint8_t);
          Eurydice_slice_copy(Eurydice_slice_subslice3(uu____15,
              uu____16,
              uu____17 + uu____18,
              uint8_t *),
            Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.public_seed, uint8_t),
            uint8_t);
          pb_curr =
            pb_curr +
              Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                  &pk_mlkem_key.ptr->header.public_seed,
                  uint8_t),
                uint8_t);
          Eurydice_slice uu____19 = pb_dst;
          size_t uu____20 = pb_curr;
          size_t uu____21 = pb_curr;
          size_t
          uu____22 =
            Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.encaps_key_hash,
                uint8_t),
              uint8_t);
          Eurydice_slice_copy(Eurydice_slice_subslice3(uu____19,
              uu____20,
              uu____21 + uu____22,
              uint8_t *),
            Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.encaps_key_hash, uint8_t),
            uint8_t);
          pb_curr =
            pb_curr +
              Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                  &pk_mlkem_key.ptr->header.encaps_key_hash,
                  uint8_t),
                uint8_t);
          Eurydice_slice uu____23 = pb_dst;
          size_t uu____24 = pb_curr;
          size_t uu____25 = pb_curr;
          size_t
          uu____26 =
            Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.private_random,
                uint8_t),
              uint8_t);
          Eurydice_slice_copy(Eurydice_slice_subslice3(uu____23,
              uu____24,
              uu____25 + uu____26,
              uint8_t *),
            Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.private_random, uint8_t),
            uint8_t);
          pb_curr =
            pb_curr +
              Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                  &pk_mlkem_key.ptr->header.private_random,
                  uint8_t),
                uint8_t);
          uu____1 = true;
          if (uu____1)
          {
            uu____3 = pb_curr;
            uu____5 = pb_dst;
            uu____4 = Eurydice_slice_len(uu____5, uint8_t);
            uu____2 = uu____3 == uu____4;
            EURYDICE_ASSERT(uu____2, "panic!");
          }
          return symcrust_common_Error_NoError;
        }
        else
        {
          uu____0 = symcrust_common_Error_InvalidArgument;
        }
        break;
      }
    case symcrust_key_Format_EncapsulationKey:
      {
        size_t uu____27 = Eurydice_slice_len(pb_dst, uint8_t);
        if (!(uu____27 != symcrust_mlkem_sizeof_format_encapsulation_key((size_t)n_rows)))
        {
          Eurydice_slice_copy(Eurydice_slice_subslice3(pb_dst,
              pb_curr,
              pb_curr + cb_encoded_vector,
              uint8_t *),
            Eurydice_array_to_subslice3(&pk_mlkem_key.ptr->header.encoded_t,
              (size_t)0U,
              cb_encoded_vector,
              uint8_t *),
            uint8_t);
          pb_curr = pb_curr + cb_encoded_vector;
          Eurydice_slice uu____28 = pb_dst;
          size_t uu____29 = pb_curr;
          size_t uu____30 = pb_curr;
          size_t
          uu____31 =
            Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                &pk_mlkem_key.ptr->header.public_seed,
                uint8_t),
              uint8_t);
          Eurydice_slice_copy(Eurydice_slice_subslice3(uu____28,
              uu____29,
              uu____30 + uu____31,
              uint8_t *),
            Eurydice_array_to_slice((size_t)32U, &pk_mlkem_key.ptr->header.public_seed, uint8_t),
            uint8_t);
          pb_curr =
            pb_curr +
              Eurydice_slice_len(Eurydice_array_to_slice((size_t)32U,
                  &pk_mlkem_key.ptr->header.public_seed,
                  uint8_t),
                uint8_t);
          uu____1 = true;
          if (uu____1)
          {
            uu____3 = pb_curr;
            uu____5 = pb_dst;
            uu____4 = Eurydice_slice_len(uu____5, uint8_t);
            uu____2 = uu____3 == uu____4;
            EURYDICE_ASSERT(uu____2, "panic!");
          }
          return symcrust_common_Error_NoError;
        }
        uu____0 = symcrust_common_Error_InvalidArgument;
        break;
      }
    default:
      {
        KRML_HOST_EPRINTF("KaRaMeL incomplete match at %s:%d\n", __FILE__, __LINE__);
        KRML_HOST_EXIT(253U);
      }
  }
  return uu____0;
}

/**
This function found in impl {core::clone::Clone for symcrust::key::Params}
*/
inline symcrust_key_Params symcrust_key_clone_61(symcrust_key_Params *self)
{
  return self[0U];
}

/**
This function found in impl {core::cmp::PartialEq<symcrust::key::Params> for symcrust::key::Params}
*/
inline bool symcrust_key_eq_d5(symcrust_key_Params *self, symcrust_key_Params *other)
{
  ptrdiff_t __self_discr = (ptrdiff_t)self[0U];
  ptrdiff_t __arg1_discr = (ptrdiff_t)other[0U];
  return __self_discr == __arg1_discr;
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice symcrust_key_atranspose_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      (size_t)0U,
      m_len,
      Eurydice_arr_bc *);
}

/**
This function found in impl {symcrust::key::PreKey2<@Slice<@Array<u16, 256: usize>>>}
*/
Eurydice_slice symcrust_key_t_fd(Eurydice_dst_6f self)
{
  size_t m_len = symcrust_key_matrix_len_fd(self);
  return
    Eurydice_slice_subslice3(Eurydice_slice_of_dst(&self.ptr->data,
        self.len,
        Eurydice_arr_bc,
        Eurydice_slice),
      m_len,
      m_len + self.ptr->header.n_rows,
      Eurydice_arr_bc *);
}

