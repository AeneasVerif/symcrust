...
)
{
  ...
  {
    uint32_t
    nBitsToEncode = symcrust_ntt_min(nBitsInCoefficient[0U], 32U - nBitsInAccumulator[0U]);
    uint32_t bitsToEncode = coefficient[0U] & ((1U << (uint32_t)nBitsToEncode) - 1U);
    coefficient[0U] = coefficient[0U] >> (uint32_t)nBitsToEncode;
    nBitsInCoefficient[0U] = nBitsInCoefficient[0U] - nBitsToEncode;
    accumulator[0U] = accumulator[0U] | bitsToEncode << (uint32_t)nBitsInAccumulator[0U];
    nBitsInAccumulator[0U] = nBitsInAccumulator[0U] + nBitsToEncode;
    if (nBitsInAccumulator[0U] == 32U)
    {
      Eurydice_slice
      uu____0 =
        Eurydice_slice_subslice2(pbDst,
          cbDstWritten[0U],
          cbDstWritten[0U] + (size_t)4U,
          uint8_t);
      uint8_t ret[4U];
      core_num__u32_8__to_le_bytes(accumulator[0U], ret);
      Eurydice_slice_copy(uu____0, Eurydice_array_to_slice((size_t)4U, ret, uint8_t), uint8_t);
      cbDstWritten[0U] = cbDstWritten[0U] + (size_t)4U;
      accumulator[0U] = 0U;
      nBitsInAccumulator[0U] = 0U;
      if (!(nBitsInCoefficient[0U] > 0U))
      {
        break;
      }
    }
    else if (!(nBitsInCoefficient[0U] > 0U))
    {
      break;
    }
  }
...
)
{
  ...
  while (true)
  {
    if (i < SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS)
    {
      ...
      {
        uint64_t
        multiplication = (uint64_t)coefficient * (uint64_t)SYMCRUST_NTT_COMPRESS_MULCONSTANT;
        coefficient =
      ...
      symcrust_ntt_SymCryptMlKemPolyElementCompressAndEncode_inner_loop(pbDst,
        &cbDstWritten,
        &accumulator,
        &nBitsInAccumulator,
        &nBitsInCoefficient,
...
)
{
  while (true)
  {
    if (nBitsInAccumulator[0U] == 0U)
    {
      uint8_t uu____0[4U];
      symcrust_ntt_slice_to_sub_array((size_t)4U, pbSrc, cbSrcRead[0U], uu____0, void *);
      accumulator[0U] = core_num__u32_8__from_le_bytes(uu____0);
      cbSrcRead[0U] = cbSrcRead[0U] + (size_t)4U;
      nBitsInAccumulator[0U] = 32U;
      uint32_t
      nBitsToDecode =
        symcrust_ntt_min(nBitsPerCoefficient - nBitsInCoefficient[0U],
          nBitsInAccumulator[0U]);
      EURYDICE_ASSERT(nBitsToDecode <= nBitsInAccumulator[0U], "panic!");
      uint32_t bitsToDecode = accumulator[0U] & ((1U << (uint32_t)nBitsToDecode) - 1U);
      accumulator[0U] = accumulator[0U] >> (uint32_t)nBitsToDecode;
      nBitsInAccumulator[0U] = nBitsInAccumulator[0U] - nBitsToDecode;
      coefficient[0U] = coefficient[0U] | bitsToDecode << (uint32_t)nBitsInCoefficient[0U];
      nBitsInCoefficient[0U] = nBitsInCoefficient[0U] + nBitsToDecode;
      if (!(nBitsPerCoefficient > nBitsInCoefficient[0U]))
      {
        return;
      }
    }
...
)
{
  ...
  while (true)
  {
    if (i < SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS)
    {
      ...
      symcrust_ntt_SymCryptMlKemPolyElementDecodeAndDecompress_inner_loop(pbSrc,
        nBitsPerCoefficient,
        &cbSrcRead,
        &accumulator,
        &nBitsInAccumulator,
...
void symcrust_ntt_SymCryptMlKemPolyElementINTTLayerC(uint16_t *peSrc, size_t k, size_t len)
{
  ...
  while (true)
  {
    if (start < (size_t)256U)
    {
      ...
      k--;
      symcrust_ntt_SymCryptMlKemPolyElementINTTLayerC_inner_loop(peSrc,
        len,
        start,
        twiddleFactor,
        twiddleFactorMont);
      start = start + (size_t)2U * len;
    }
    else
    {
      break;
...
)
{
  ...
  while (true)
  {
    if (i < SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)2U)
    {
...
void symcrust_ntt_SymCryptMlKemPolyElementNTTLayerC(uint16_t *peSrc, size_t k, size_t len)
{
  ...
  while (true)
  {
    if (start < (size_t)256U)
    {
      ...
      uint32_t twiddleFactorMont = (uint32_t)symcrust_ntt_MlKemZetaBitRevTimesRTimesNegQInvModR[k];
      k++;
      symcrust_ntt_SymCryptMlKemPolyElementNTTLayerC_inner_loop(peSrc,
        len,
        start,
        twiddleFactor,
        twiddleFactorMont);
...
)
{
  ...
  while (true)
  {
    if (j < (size_t)4U)
    {
      uint32_t coefficient = sampleBits[0U] & 63U;
      sampleBits[0U] = sampleBits[0U] >> 6U;
...
)
{
  ...
  while (true)
  {
    if (j < (size_t)8U)
    {
      uint32_t coefficient = sampleBits[0U] & 15U;
      sampleBits[0U] = sampleBits[0U] >> 4U;
  ...
  if (eta == 3U)
  {
    ...
    while (true)
    {
      if (i < SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS)
      {
        ...
        symcrust_ntt_SymCryptMlKemPolyElementSampleCBDFromBytes_then_inner_loop(peDst,
          i,
          &sampleBits);
        i++;
      }
  ...
  else
  {
    ...
    while (true)
    {
      if (i < SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS)
      {
        ...
        sampleBits = (sampleBits & 1431655765U) + (sampleBits >> 1U & 1431655765U);
        symcrust_ntt_SymCryptMlKemPolyElementSampleCBDFromBytes_else_inner_loop(peDst,
          i,
          &sampleBits);
        i++;
      }
...
symcrust_ntt_SymCryptMlKemPolyElementSampleNTTFromShake128(uint8_t *pState, uint16_t *peDst)
{
  ...
  while (true)
  {
    if (i < SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS)
    {
      ...
      {
        symcrust_ntt_SymCryptShake128Extract(pState,
