...
{
  size_t nRows = pmSrc.nRows;
  EURYDICE_ASSERT(nRows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(nRows <= SYMCRUST_NTT_MATRIX_MAX_NROWS, "panic!");
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    for (size_t j = i + (size_t)1U; j < nRows; j++)
    {
      pmSrc = symcrust_ntt__symcrust__ntt__MATRIX__a___swap(pmSrc, i * nRows + j, j * nRows + i);
    }
    pmSrc = pmSrc;
  }
  return pmSrc;
...
{
  size_t nRows = pmSrc1.nRows;
  EURYDICE_ASSERT(nRows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(nRows <= SYMCRUST_NTT_MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pvSrc2, uint16_t [256U]) == nRows, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pvDst, uint16_t [256U]) == nRows, "panic!");
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    for (size_t j = (size_t)0U; j < nRows; j++)
    {
      pmSrc1 =
        symcrust_ntt_SymCryptMlKemPolyElementMulAndAccumulate_aux(pmSrc1,
          nRows,
          i,
          j,
          Eurydice_slice_index(pvSrc2, i, uint16_t [256U], uint16_t (*)[256U]),
          paTmp);
    }
    pmSrc1 = pmSrc1;
    symcrust_ntt_SymCryptMlKemMontgomeryReduceAndAddPolyElementAccumulatorToPolyElement(/* write accumulator to dest and zero accumulator */
        paTmp
      ,
      Eurydice_slice_index(pvDst, i, uint16_t [256U], uint16_t (*)[256U]));
  }
  return pmSrc1;
}
...
)
{
  ...
  EURYDICE_ASSERT(cbSrcRead
    == (size_t)(nBitsPerCoefficient * ((uint32_t)SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS / 8U)),
    "panic!");
  return symcrust_ntt_MLKEM_ERROR_NO_ERROR;
}
...
{
  size_t nRows = Eurydice_slice_len(pvSrc1, uint16_t [256U]);
  EURYDICE_ASSERT(nRows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(nRows <= SYMCRUST_NTT_MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pvSrc2, uint16_t [256U]) == nRows, "panic!");
  EURYDICE_ASSERT(Eurydice_slice_len(pvDst, uint16_t [256U]) == nRows, "panic!");
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    symcrust_ntt_SymCryptMlKemPolyElementAdd(Eurydice_slice_index(pvSrc1,
        i,
        uint16_t [256U],
        uint16_t (*)[256U]),
      Eurydice_slice_index(pvSrc2, i, uint16_t [256U], uint16_t (*)[256U]),
...
{
  size_t nRows = Eurydice_slice_len(pvSrc, uint16_t [256U]);
  EURYDICE_ASSERT(nRows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(nRows <= SYMCRUST_NTT_MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(nBitsPerCoefficient > 0U, "panic!");
  EURYDICE_ASSERT(nBitsPerCoefficient <= 12U, "panic!");
  EURYDICE_ASSERT(cbDst
    ==
      nRows
      * (size_t)(nBitsPerCoefficient * ((uint32_t)SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS / 8U)),
    "panic!");
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    ...
    pbDst_index =
      ...
      * (size_t)nBitsPerCoefficient
      * (SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
    uint16_t *uu____0 = Eurydice_slice_index(pvSrc, i, uint16_t [256U], uint16_t (*)[256U]);
    uint32_t uu____1 = nBitsPerCoefficient;
    symcrust_ntt_SymCryptMlKemPolyElementCompressAndEncode(uu____0,
      uu____1,
      Eurydice_slice_subslice_from(pbDst, pbDst_index, uint8_t, size_t));
  }
}
...
{
  size_t nRows = Eurydice_slice_len(pvDst, uint16_t [256U]);
  EURYDICE_ASSERT(nRows > (size_t)0U, "panic!");
  EURYDICE_ASSERT(nRows <= SYMCRUST_NTT_MATRIX_MAX_NROWS, "panic!");
  EURYDICE_ASSERT(nBitsPerCoefficient > 0U, "panic!");
  EURYDICE_ASSERT(nBitsPerCoefficient <= 12U, "panic!");
  EURYDICE_ASSERT(cbSrc
    ==
      nRows
      * (size_t)nBitsPerCoefficient
      * (SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U),
  ...
  while (i < nRows)
  {
    ...
    pbSrc_index =
      ...
      * (size_t)nBitsPerCoefficient
      * (SYMCRUST_NTT_MLWE_POLYNOMIAL_COEFFICIENTS / (size_t)8U);
    Eurydice_slice uu____0 = Eurydice_slice_subslice_from(pbSrc, pbSrc_index, uint8_t, size_t);
    symcrust_ntt_MLKEM_ERROR
    scError =
      symcrust_ntt_SymCryptMlKemPolyElementDecodeAndDecompress(uu____0,
        nBitsPerCoefficient,
        Eurydice_slice_index(pvDst, i, uint16_t [256U], uint16_t (*)[256U]));
    if (!(scError == symcrust_ntt_MLKEM_ERROR_NO_ERROR))
    {
      i++;
      continue;
    }
...
void symcrust_ntt_SymCryptMlKemVectorINTTAndMulR(Eurydice_slice pvSrc)
{
  ...
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    symcrust_ntt_SymCryptMlKemPolyElementINTTAndMulR(Eurydice_slice_index(pvSrc,
        i,
        uint16_t [256U],
        uint16_t (*)[256U]));
  }
...
)
{
  ...
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    symcrust_ntt_SymCryptMlKemPolyElementMulAndAccumulate(Eurydice_slice_index(pvSrc1,
        i,
        uint16_t [256U],
        uint16_t (*)[256U]),
      Eurydice_slice_index(pvSrc2, i, uint16_t [256U], uint16_t (*)[256U]),
...
void symcrust_ntt_SymCryptMlKemVectorMulR(Eurydice_slice pvSrc, Eurydice_slice pvDst)
{
  ...
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    symcrust_ntt_SymCryptMlKemPolyElementMulR(Eurydice_slice_index(pvSrc,
        i,
        uint16_t [256U],
        uint16_t (*)[256U]),
      Eurydice_slice_index(pvDst, i, uint16_t [256U], uint16_t (*)[256U]));
...
void symcrust_ntt_SymCryptMlKemVectorNTT(Eurydice_slice pvSrc)
{
  ...
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    symcrust_ntt_SymCryptMlKemPolyElementNTT(Eurydice_slice_index(pvSrc,
        i,
        uint16_t [256U],
        uint16_t (*)[256U]));
  }
...
)
{
  ...
  for (size_t i = (size_t)0U; i < nRows; i++)
  {
    symcrust_ntt_SymCryptMlKemPolyElementSub(Eurydice_slice_index(pvSrc1,
        i,
        uint16_t [256U],
        uint16_t (*)[256U]),
      Eurydice_slice_index(pvSrc2, i, uint16_t [256U], uint16_t (*)[256U]),
